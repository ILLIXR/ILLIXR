...
void offload_rendering_client::setup(VkRenderPass render_pass, uint32_t subpass,
    ...
    command_pool      = vulkan::create_command_pool(display_provider_->vk_device_, display_provider_->queues_[vulkan::queue::GRAPHICS].family);
    ...
    for (auto& frame : avvk_color_frames_) {
        for (auto& eye : frame) {
            ...
            vulkan::end_one_time_command(display_provider_->vk_device_, command_pool, display_provider_->queues_[vulkan::queue::GRAPHICS], cmd_buf);
        }
    }
    ...
    for (size_t i = 0; i < avvk_color_frames_.size(); i++) {
        for (auto eye = 0; eye < 2; eye++) {
            ...
            layout_transition_start_cmd_bufs_[i][eye] = vulkan::create_command_buffer(display_provider_->vk_device_, command_pool);
            ...
            layout_transition_end_cmd_bufs_[i][eye] = vulkan::create_command_buffer(display_provider_->vk_device_, command_pool);
...
void offload_rendering_client::transition_layout(VkCommandBuffer cmd_buf, AVFrame* frame, VkImageLayout old_layout, VkImageLayout new_layout) {
...
void offload_rendering_client::_p_one_iteration() {
    ...
    for (auto eye = 0; eye < 2; eye++) {
       ...
       if (use_depth_) {
           ...
           ret =
               nppiNV12ToYUV420_8u_P2P3R(pSrc_depth, decode_out_depth_frames_[eye]->linesize[0], pDst, dst_linesizes, roi);
           assert(ret == NPP_SUCCESS);
           auto dst_depth = reinterpret_cast<Npp8u*>(decode_converted_depth_frames_[eye]->data[0]);
           ret = nppiYUV420ToBGR_8u_P3C4R(pDst, dst_linesizes, dst_depth, decode_converted_depth_frames_[eye]->linesize[0],
                                                     roi);
           assert(ret == NPP_SUCCESS);
       }
...
void offload_rendering_client::ffmpeg_init_device() {
    ...
    if (display_provider_->queues_.find(vulkan::queue::DEDICATED_TRANSFER) != display_provider_->queues_.end()) {
        vulkan_hwdev_ctx->queue_family_tx_index = static_cast<int>(display_provider_->queues_[vulkan::queue::DEDICATED_TRANSFER].family);
        vulkan_hwdev_ctx->nb_tx_queues          = 1;
    }
...
void offload_rendering_client::ffmpeg_init_buffer_pool() {
    ...
    for (size_t i = 0; i < buffer_pool_->image_pool.size(); i++) {
        for (size_t eye = 0; eye < 2; eye++) {
            ...
            vk_frame->sem[0]       = vulkan::create_timeline_semaphore(display_provider_->vk_device_, 0, &export_semaphore_create_info);
            ...
            if (use_depth_) {
                ...
                vk_depth_frame->offset[0]       = static_cast<ptrdiff_t>(buffer_pool_->depth_image_pool[i][eye].allocation_info.offset);
                vk_depth_frame->queue_family[0] = display_provider_->queues_[vulkan::queue::GRAPHICS].family;

                vk_depth_frame->sem[0] = vulkan::create_timeline_semaphore(display_provider_->vk_device_, 0, &export_semaphore_create_info);
    ...
    for (size_t eye = 0; eye < 2; eye++) {
        ...
        if (use_depth_) {
            ...
            decode_out_depth_frames_[eye]->width           = static_cast<int>(buffer_pool_->depth_image_pool[0][0].image_info.extent.width);
            decode_out_depth_frames_[eye]->height          = static_cast<int>(buffer_pool_->depth_image_pool[0][0].image_info.extent.height);
            ...
            decode_converted_depth_frames_[eye]->width         = static_cast<int>(buffer_pool_->depth_image_pool[0][0].image_info.extent.width);
            decode_converted_depth_frames_[eye]->height        = static_cast<int>(buffer_pool_->depth_image_pool[0][0].image_info.extent.height);
