...
stereo_surface_decoder::stereo_surface_decoder(int width, int height, bool is_depth, bool is_10bit)
    ...
    spdlog::get("illixr")->info(
            "[stereo_surface_decoder] COMBINED_ENCODING: created {} {} combined decoder "
            "{}x{} (per-eye padded {}x{}, original {}x{})",
            is_10bit_ ? "10-bit" : "8-bit",
            is_depth_  ? "DEPTH"  : "COLOR",
            combined_w, padded_height_,
    ...
    spdlog::get("illixr")->info(
            "[stereo_surface_decoder] Created {} {} decoder {}x{} (padded from {}x{})",
            is_10bit_ ? "10-bit" : "8-bit",
            is_depth_  ? "DEPTH"  : "COLOR",
            padded_width_, padded_height_,
            original_width_, original_height_);
...
bool stereo_surface_decoder::initialize() {
    ...
    spdlog::get("illixr")->info(
            "[stereo_surface_decoder] COMBINED_ENCODING initialized (AImageReader/Vulkan path)");
    return true;
#else
...
bool stereo_surface_decoder::queue_encoded_data(int eye, const uint8_t* data, size_t size,
    ...
    return combined_decoder_->queue_encoded_data(
            data, size, timestamp_us, is_keyframe, frame_number);
#else
    if (eye == 0) {
...
dual_frames stereo_surface_decoder::get_current_frame(time_point render_time) {
    ...
    if (!combined_decoder_->has_decoded_frames()) {
        ...
        if (++wait_count % 1000 == 1) {
            spdlog::get("illixr")->info(
                    "[stereo_surface_decoder][{}][combined] Waiting for first decoded frame "
                    "(released={}, queued={}, dropped={}, count={})",
                    is_depth_ ? "depth" : "color",
                    combined_decoder_->get_frames_released(),
                    combined_decoder_->get_frames_queued(),
                    combined_decoder_->get_frames_dropped(),
                    wait_count);
        }
        return dual_frames{};
    }
    ...
    return dual_frames{render_time, combined_buf, combined_buf,
                       combined_width, padded_height_, frame_number};
    ...
    if (!left_decoder_->has_decoded_frames() || !right_decoder_->has_decoded_frames()) {
        ...
        if (++wait_count % 100 == 1) {
            spdlog::get("illixr")->info("[stereo_surface_decoder][{}] Waiting for first decoded frames "
                                        ...
                                        is_depth_ ? "depth" : "color",
                                        left_decoder_->get_frames_decoded(),
                                        right_decoder_->get_frames_decoded(),
                                        left_decoder_->get_frames_released(),
                                        right_decoder_->get_frames_released(),
                                        left_decoder_->get_frames_queued(),
                                        right_decoder_->get_frames_queued(),
                                        left_decoder_->get_frames_dropped(),
                                        right_decoder_->get_frames_dropped(),
                                        wait_count);
        }
        return dual_frames{};
    }
    ...
    if (acquire_count <= 5 || acquire_count % 200 == 0) {
        spdlog::get("illixr")->info("[stereo_surface_decoder][{}] acquire #{}: "
                                    ...
                                    is_depth_ ? "depth" : "color",
                                    acquire_count,
                                    static_cast<void*>(left_buf),
                                    static_cast<void*>(right_buf),
                                    frame_number);
    }

    if (left_buf == nullptr || right_buf == nullptr) {
        if (left_buf)  AHardwareBuffer_release(left_buf);
        if (right_buf) AHardwareBuffer_release(right_buf);
        return dual_frames{};
    }

    return dual_frames{render_time, left_buf, right_buf,
                       padded_width_, padded_height_, frame_number};
#endif // COMBINED_ENCODING
}
...
#ifdef COMBINED_ENCODING
    if (combined_decoder_) { combined_decoder_->flush(); }
#else
    left_decoder_->flush();
