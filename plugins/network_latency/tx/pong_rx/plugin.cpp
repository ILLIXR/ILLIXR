...
void network_latency_pong_rx::process_pong(const switchboard::ptr<const latency_pong>& pong) {
    ...
    spdlog::get("illixr")->warn("Rx time: {}ns", receive_time);
    ...
    if (rtt_ms > 500.0 || rtt_ms < 0.0) {
        spdlog::get("illixr")->warn("[network_latency_pong_rx] Rejecting implausible RTT={:.3f}ms", rtt_ms);
        return;
    }
    ...
    if (clock_offset_initialized_ && std::abs(clock_offset_ms - smoothed_clock_offset_ms_) > 10.0) {
        spdlog::get("illixr")->warn("[network_latency_pong_rx] Rejecting outlier offset={:.3f}ms "
                                    "(smoothed={:.3f}ms)",
    ...
    if (!clock_offset_initialized_) {
        ...
        smoothed_clock_offset_ms_ = alpha * clock_offset_ms + (1.0 - alpha) * smoothed_clock_offset_ms_;
        smoothed_rtt_ms_          = alpha * rtt_ms + (1.0 - alpha) * smoothed_rtt_ms_;
    }
    // Publish the latency result
    result_writer_.put(result_writer_.allocate<network_latency_result>(
        network_latency_result{pong->sequence_number, rtt_ms, c2s_ms, s2c_ms, clock_offset_ms, smoothed_clock_offset_ms_,
                               smoothed_rtt_ms_, receive_time}));

    spdlog::get("illixr")->info("[network_latency_pong_rx] seq={}: RTT={:.3f}ms (smooth={:.3f}ms) "
                                "offset={:.3f}ms (smooth={:.3f}ms)",
                                pong->sequence_number, rtt_ms, smoothed_rtt_ms_.load(), clock_offset_ms,
                                smoothed_clock_offset_ms_.load());
}
