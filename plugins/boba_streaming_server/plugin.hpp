...
private:
    ...
    std::uint64_t modal_texture_id(const std::vector<std::uint8_t>& rgba, std::uint32_t width,
                                   std::uint32_t height) const;
    void publish_modal_texture_if_needed(const data_format::boba_modal_overlay& modal,
