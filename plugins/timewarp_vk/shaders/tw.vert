...
layout(binding = 0) uniform UniformBufferObject {
    mat4 TimeWarpStartTransform[2];
    mat4 TimeWarpEndTransform[2];
} ubo;
...
void main() {
    ...
    vec3 startUv0 = (ubo.TimeWarpStartTransform[eye.index] * vec4( vertexUv0, -1, 1 )).xyz;
    vec3 startUv1 = (ubo.TimeWarpStartTransform[eye.index] * vec4( vertexUv1, -1, 1 )).xyz;
    vec3 startUv2 = (ubo.TimeWarpStartTransform[eye.index] * vec4( vertexUv2, -1, 1 )).xyz;

    vec3 endUv0 = (ubo.TimeWarpEndTransform[eye.index] * vec4( vertexUv0, -1, 1 )).xyz;
    vec3 endUv1 = (ubo.TimeWarpEndTransform[eye.index] * vec4( vertexUv1, -1, 1 )).xyz;
    vec3 endUv2 = (ubo.TimeWarpEndTransform[eye.index] * vec4( vertexUv2, -1, 1 )).xyz;
