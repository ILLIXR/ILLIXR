...
public:
    ...
    /**
     * @brief Indicates this sink does not make use of the rendering pipeline in order for the access masks of the layout transitions to be set properly
     */
...
private:
    ...
    void enqueue_for_network_send(fast_pose_type& pose) {
        ...
        if (pass_depth) {
            frames_topic.put(std::make_shared<compressed_frame>(
                encode_out_color_packets[0], encode_out_color_packets[1],
                encode_out_depth_packets[0], encode_out_depth_packets[1],
                pose, timestamp, nalu_only));
        } else {
            frames_topic.put(std::make_shared<compressed_frame>(
                encode_out_color_packets[0], encode_out_color_packets[1],
                pose, timestamp, nalu_only));
