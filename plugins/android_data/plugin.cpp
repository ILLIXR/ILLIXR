...
#ifndef NDEBUG
            spdlog::get("illixr")->debug("IMU Values : accel %f %f %f %f %f %f", accel_[0], accel_[1], accel_[2], gyro_[0],
                                         gyro_[1], gyro_[2]);
#endif
        }
        default:;
...
void* android_data::android_run_thread(void* ptr) {
    ...
    while (ret != ALOOPER_POLL_ERROR) {
        ...
        ASensorEvent event;
        while (ASensorEventQueue_getEvents(d->event_queue, &event, 2) > 0) {
            lock_.lock();
            switch (event.type) {
            case ASENSOR_TYPE_ACCELEROMETER:
                accel_[0] = event.acceleration.x;
...
AImageReader* android_data::create_jpeg_reader() {
    ...
    media_status_t status = AImageReader_new(IMAGE_WIDTH, IMAGE_HEIGHT, AIMAGE_FORMAT_YUV_420_888, 10, &reader);
...
void android_data::on_capture_failed(void* context, ACameraCaptureSession* session, ACaptureRequest* request,
...
std::string android_data::get_back_facing_cam_id() {
    ...
    for (int i = 0; i < cameraIds->numCameras; ++i) {
        ...
        if (res != ACAMERA_OK)
            continue;
#ifndef NDEBUG
        auto pose_ref = static_cast<acamera_metadata_enum_acamera_lens_pose_reference>(pose_reference.data.u8[0]);

        spdlog::get("illixr")->debug("Pose reference is = %d and facing = %d", static_cast<int>(pose_ref),
        ...
        if (camera_found) {
            ...
            ACameraMetadata_getConstEntry(metadataObj, ACAMERA_LENS_INTRINSIC_CALIBRATION, &intrinsics);
#ifndef NDEBUG
            for (int x = 0; x < intrinsics.count; ++x) {
                spdlog::get("illixr")->debug("Intrinsics total = %d : %f : index %d", intrinsics.count, intrinsics.data.f[x],
                                             x);
            }
#endif
            ...
            for (int x = 0; x < distortion.count; ++x) {
                spdlog::get("illixr")->debug("distortion  = %d : %f : index %d", distortion.count, distortion.data.f[x], x);
            }
#endif
            ...
            for (int x = 0; x < rotation.count; ++x) {
                spdlog::get("illixr")->debug("rotation total = %d : %f : index %d", rotation.count, rotation.data.f[x], x);
            }
#endif
            ...
            for (int x = 0; x < translation.count; ++x) {
                spdlog::get("illixr")->debug("translation total = %d : %f : index %d", translation.count, translation.data.f[x],
                                             x);
            }
#endif
            break;
...
void android_data::init_cam() {
    ...
    // Create the session
    ACameraDevice_createCaptureSession(camera_device_, outputs_, &session_state_callbacks_, &capture_session_);
#ifndef NDEBUG
    camera_status_t status =
        ACameraCaptureSession_setRepeatingRequest(capture_session_, &capture_callbacks_, 1, &request_, nullptr);
...
void android_data::_p_one_iteration() {
    ...
    //           std::to_string(gyro[2]) + "," + std::to_string(accel[0]) + ","  + std::to_string(accel[1]) + "," +
    //           std::to_string(accel[2])  + "\n";
    ...
    // ANDROID_LOG("TIME = %lf and cam_time %llu",duration_2_double(std::chrono::nanoseconds(cam_time - *_m_first_cam_time)),
    // cam_time);
    // _m_cam.put(_m_cam.allocate<cam_type>({cam_time_point, ir_left, ir_right}));
    imu_.put(imu_.allocate<data_format::imu_type>(data_format::imu_type{time_point{curr_time}, cur_gyro, cur_accel}));
    cam_.put(cam_.allocate<data_format::binocular_cam_type>(
        data_format::binocular_cam_type{time_point{curr_time}, ir_left.clone(), ir_right.clone()}));
    auto stop     = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
#ifndef NDEBUG
    spdlog::get("illixr")->debug("duration: %f", duration_to_double(duration));
