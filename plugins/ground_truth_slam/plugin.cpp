...
[[maybe_unused]] ground_truth_slam::ground_truth_slam(const std::string& name, phonebook* pb)
    ...
    , sensor_data_{load_data<pose::head_pose_type>("state_groundtruth_estimate0", "ground_truth_slam", &read_data, switchboard_)}
...
void ground_truth_slam::feed_ground_truth(const switchboard::ptr<const imu_type>& datum) {
    ...
    switchboard::ptr<pose::head_pose_type> true_pose =
        true_pose_.allocate<pose::head_pose_type>(pose::head_pose_type{time_point{datum->time}, it->second.position, it->second.orientation});
