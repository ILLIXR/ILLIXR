...
#include "../include/illixr/gl_util/lib/stb_image.h"
...
bool IBL::loadFromEquirect(Path const& path) {
    ...
    mIndirectLight = IndirectLight::Builder().reflections(mTexture).intensity(IBL_INTENSITY).build(mEngine);
...
bool IBL::loadFromDirectory(const utils::Path& path) {
    ...
    mIndirectLight =
        IndirectLight::Builder().reflections(mTexture).irradiance(3, mBands).intensity(IBL_INTENSITY).build(mEngine);
...
bool IBL::loadCubemapLevel(filament::Texture** texture, Texture::PixelBufferDescriptor* outBuffer, uint32_t* dim,
    ...
    *dim                  = size;
    ...
    Texture::PixelBufferDescriptor buffer(malloc(faceSize * 6), faceSize * 6, Texture::Format::RGB,
    ...
    for (size_t j = 0; j < 6; j++) {
        ...
        if (w != h || w != size) {
            std::cerr << "Face " << faceName << "has a wrong size " << w << " x " << h << ", instead of " << size << " x "
                      << size << std::endl;
            success = false;
            break;
        }
