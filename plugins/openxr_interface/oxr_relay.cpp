...
void oxr_relay::start() {
    ...
    combined_pose_capture_writer_.emplace(switchboard_->get_writer<data_format::pose::combined_pose>("combined_pose_capture"));
#endif
...
void oxr_relay::_p_one_iteration() {
    ...
    auto log_iter_time = [&](const char* exit_point) {
        auto ms = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - iter_start).count() /
            1000.0;
        if (ms > TIME_CUTOFF)
    ...
    if (!time_offsets_calibrated_ || now - last_offset_calibration_ > std::chrono::seconds(30)) {
    ...
    if (!xr_time_verified_) {
        ...
        if (diff_ms > 1000) {
            spdlog::get("illixr")->warn("oxr_relay: XrTime ({}) differs from CLOCK_BOOTTIME ({}) "
    ...
    if (XR_SUCCEEDED(get_head_pose(pose_time, &head_pose))) {
        ...
        if (head_pose.relation_flags & pose::XRT_SPACE_RELATION_ORIENTATION_VALID_BIT &&
            ...
            // spdlog::get("illixr")->debug("pose valid, writing {}, {}, {}, {}", headPose.orientation.w,
            // headPose.orientation.x, headPose.orientation.y, headPose.orientation.z);
        } else {
...
bool oxr_relay::init_hand_tracking() {
    ...
    OXR(xrGetInstanceProcAddr(instance_, "xrCreateHandTrackerEXT", (PFN_xrVoidFunction*) &xr_create_hand_tracker_))
    OXR(xrGetInstanceProcAddr(instance_, "xrDestroyHandTrackerEXT", (PFN_xrVoidFunction*) &xr_destroy_hand_tracker_))
    OXR(xrGetInstanceProcAddr(instance_, "xrLocateHandJointsEXT", (PFN_xrVoidFunction*) &xr_locate_hand_joints_))
    ...
    for (int hand_idx = 0; hand_idx < 2; hand_idx++) {
        ...
        if (ht_should_log) {
            spdlog::get("illixr")->debug("[hand_tracking] {} xrLocateHandJointsEXT result={} isActive={}", hand_names[hand_idx],
                                         static_cast<int>(result), locations.isActive ? "TRUE" : "FALSE");
        }
        ...
        if (XR_SUCCEEDED(result) && locations.isActive) {
            ...
            if (ht_should_log) {
                ...
                spdlog::get("illixr")->debug(
                    ...
                    hand_names[hand_idx], hand_states[hand_idx]->is_active, wrist.relation.pose.position.x,
                    wrist.relation.pose.position.y, wrist.relation.pose.position.z,
                    static_cast<unsigned>(wrist.relation.relation_flags), index_tip.relation.pose.position.x,
                    index_tip.relation.pose.position.y, index_tip.relation.pose.position.z,
            ...
            if (XR_FAILED(result)) {
                spdlog::get("illixr")->warn("[hand_tracking] {} xrLocateHandJointsEXT FAILED result={}", hand_names[hand_idx],
    ...
    if (should_log) {
        ...
        if (current_palm_poses_.is_valid()) {
            ...
            for (int h = 0; h < 2; h++) {
                ...
                spdlog::get("illixr")->debug("[palm_pose] {} | "
                                             "pos=({:.3f},{:.3f},{:.3f}) | "
                                             "ori=({:.3f},{:.3f},{:.3f},{:.3f})",
                                             hand_names[h], pd.pose.position.x, pd.pose.position.y, pd.pose.position.z,
                                             pd.pose.orientation.x, pd.pose.orientation.y, pd.pose.orientation.z,
                                             pd.pose.orientation.w);
            }
        } else {
            ...
            for (int h = 0; h < 2; h++) {
                ...
                for (int t = 0; t < pose::NUM_INTERACTION_POSES; t++) {
                    ...
                    if (type_enums[t] == pose::POKE) {
                        spdlog::get("illixr")->debug("[hand_interaction] {} {} | pos=({:.3f},{:.3f},{:.3f})", hand_names[h],
                                                     type_names[t], ip.pose.position.x, ip.pose.position.y, ip.pose.position.z);
                    } else {
                        spdlog::get("illixr")->debug(
                            "[hand_interaction] {} {} | pos=({:.3f},{:.3f},{:.3f}) | value={:.3f} ready={}", hand_names[h],
                            type_names[t], ip.pose.position.x, ip.pose.position.y, ip.pose.position.z, ip.value,
                            ip.ready ? "true" : "false");
                    }
                }
            }
...
bool oxr_relay::init_hand_interaction() {
    ...
    const char* pose_suffixes[pose::NUM_INTERACTION_POSES] = {"/input/aim/pose", "/input/grip/pose", "/input/pinch_ext/pose",
                                                              "/input/poke_ext/pose"};
    const char* value_suffixes[3] = {"/input/aim_activate_ext/value", "/input/grasp_ext/value", "/input/pinch_ext/value"};
    const char* ready_suffixes[3] = {"/input/aim_activate_ext/ready_ext", "/input/grasp_ext/ready_ext",
    ...
    // Profile 2: KHR simple controller (generic fallback)
    {
        ...
        if (XR_FAILED(r)) {
            spdlog::get("illixr")->warn("Could not suggest simple_controller bindings ({})", static_cast<int>(r));
        } else {
    ...
    if (XR_FAILED(attach_result)) {
        spdlog::get("illixr")->error("Failed to attach hand interaction action set: {}", static_cast<int>(attach_result));
...
bool oxr_relay::suggest_controller_bindings() {
    ...
    if (XR_FAILED(result)) {
        spdlog::get("illixr")->error("Could not create Oculus Touch interaction profile path: {}", static_cast<int>(result));
        return false;
    }
    ...
    if (XR_FAILED(result)) {
        spdlog::get("illixr")->error("xrSuggestInteractionProfileBindings(Oculus Touch) failed: {}", static_cast<int>(result));
        return false;
    }
...
void oxr_relay::update_hand_interaction(XrTime predicted_time) {
    ...
    for (int h = 0; h < 2; h++) {
        ...
        for (int p = 0; p < pose::NUM_INTERACTION_POSES; p++) {
            ...
            if (p < 3) { // AIM=0, GRIP=1, PINCH=2
                ...
                if (XR_SUCCEEDED(xrGetActionStateFloat(session_, &float_info, &float_state)) && float_state.isActive) {
                ...
                if (XR_SUCCEEDED(xrGetActionStateBoolean(session_, &bool_info, &bool_state)) && bool_state.isActive) {
            ...
            XrResult locate_result    = xrLocateSpace(interaction_pose_spaces_[h][p], local_space_, predicted_time, &space_loc);
            auto     t4               = std::chrono::steady_clock::now();
            auto     ms1              = std::chrono::duration_cast<std::chrono::microseconds>(t4 - t3).count() / 1000.0;
            if (ms1 > TIME_CUTOFF)
                spdlog::get("illixr")->warn("[oxr_timing] xrLocateSpace (interaction {}/{}) took {:.2f}ms", hand_names[h],
                                            type_names[p], ms1);
    ...
    for (int h = 0; h < 2; h++) {
        ...
        XrActionStatePose pose_state = {XR_TYPE_ACTION_STATE_POSE};
        if (XR_FAILED(xrGetActionStatePose(session_, &get_info, &pose_state)) || !pose_state.isActive) {
...
bool oxr_relay::query_controller_float(XrAction action, XrPath hand_path, float threshold, quest_controller_button* button) {
    *button = quest_controller_button{};
    XrActionStateGetInfo get_info{XR_TYPE_ACTION_STATE_GET_INFO};
    get_info.action        = action;
    get_info.subactionPath = hand_path;
...
bool oxr_relay::query_controller_hand(std::size_t hand_index, XrTime sample_time, quest_hand_controller* hand) {
    *hand                     = quest_hand_controller{};
    ...
    if (!query_controller_boolean(controller_trigger_click_action_, hand_subaction_paths_[hand_index], &trigger_click) ||
        ...
        !query_controller_boolean(controller_secondary_click_action_, hand_subaction_paths_[hand_index], &hand->secondary) ||
...
void oxr_relay::publish_boba_input(XrTime predicted_time, XrDuration predicted_period, XrBool32 should_render,
    ...
    const bool pose_valid =
        (view_flags & XR_VIEW_STATE_POSITION_VALID_BIT) != 0 && (view_flags & XR_VIEW_STATE_ORIENTATION_VALID_BIT) != 0;
...
void oxr_relay::calibrate_time_offsets() {
    ...
    for (int i = 0; i < NUM_SAMPLES; i++) {
        ...
        int64_t before_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(before_sys.time_since_epoch()).count();
        int64_t after_ns  = std::chrono::duration_cast<std::chrono::nanoseconds>(after_sys.time_since_epoch()).count();
