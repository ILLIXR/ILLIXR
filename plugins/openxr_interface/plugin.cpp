...
void oxr_interface::_p_thread_setup() {
    ...
    if (!renderer_->initialize(vk_instance_, vk_physical_device_, vk_device_,
                               vk_queue_, vk_queue_family_,
...
void oxr_interface::init_xr() {
    ...
    if (oxr_relay_->hand_tracking_supported_) sp.next = &ht_props;
...
void oxr_interface::create_session() {
    ...
    OXR(xrGetInstanceProcAddr(instance_, "xrCreateVulkanInstanceKHR",
                              (PFN_xrVoidFunction*)&xrCreateVulkanInstanceKHR))
    OXR(xrGetInstanceProcAddr(instance_, "xrGetVulkanGraphicsDevice2KHR",
                              (PFN_xrVoidFunction*)&xrGetVulkanGraphicsDevice2KHR))
    OXR(xrGetInstanceProcAddr(instance_, "xrCreateVulkanDeviceKHR",
    ...
    spdlog::get("illixr")->info("Vulkan required: min {}.{}, max {}.{}",
                                XR_VERSION_MAJOR(vk_reqs.minApiVersionSupported),
    ...
    OXR(xrEnumerateViewConfigurationViews(instance_, system_id_,
...
void oxr_interface::run_frame() {
    ...
    XrCompositionLayerSpaceWarpInfoFB spacewarp_infos[2]{
        ...
        if (current_frames_ && current_frames_->is_valid()) {
                ...
                if (current_frames_->pose_id != 0) {
                    ...
                    if (oxr_relay_->get_pose_history(current_frames_->pose_id,
                        ...
                        spdlog::get("illixr")->info(
                                ...
                                "initial_pose=({:.3f},{:.3f},{:.3f};{:.3f},{:.3f},{:.3f},{:.3f};{:.3f},{:.3f},{:.3f};{:.3f},{:.3f},{:.3f}) "
                                ...
                                current_frames_->frame_number,
                                history_entry.pose.pose.position.x,
                                history_entry.pose.pose.position.y,
                                history_entry.pose.pose.position.z,
                                history_entry.pose.pose.orientation.w,
                                history_entry.pose.pose.orientation.x,
                                history_entry.pose.pose.orientation.y,
                                history_entry.pose.pose.orientation.z,
                                history_entry.pose.linear_velocity.x,
                                history_entry.pose.linear_velocity.y,
                                history_entry.pose.linear_velocity.z,
                                history_entry.pose.angular_velocity.x,
                                history_entry.pose.angular_velocity.y,
                                history_entry.pose.angular_velocity.z,
                                ...
                                current_frames_->pose[0].orientation.w,
                                current_frames_->pose[0].orientation.x,
                                current_frames_->pose[0].orientation.y,
                                current_frames_->pose[0].orientation.z,
                                loc.pose.position.x,
                                loc.pose.position.y,
                                loc.pose.position.z,
                                ...
                                history_entry.xr_time,
                                now_xr,
                                static_cast<double>(now_xr -
                                                    static_cast<int64_t>(history_entry.xr_time))
                                / 1'000'000.0,
                                static_cast<double>(age_ns) / 1'000'000.0,
                                current_frames_->encode_time);
                    } else {
                        spdlog::get("illixr")->debug(
                                "[pose_tracking] frame pose_id={} not found in history "
            ...
            for (int eye = 0; eye < 2; eye++) {
                ...
                if (use_depth_ && depth_extension_supported_ &&
                    current_frames_->has_valid_depth() &&
                    ...
                    renderer_->render_eye_depth(eye, depth_vk_image, depth_format_,
                    ...
                    if (spacewarp_supported_
                        && current_frames_->has_valid_motion_vectors()
                        ...
                        renderer_->render_eye_motion_vec(eye, mv_vk_image,
                                                         mv_sc.width, mv_sc.height);
                        ...
                        sw.motionVectorSubImage.imageRect.extent = {
                                static_cast<int32_t>(mv_sc.width),
                        ...
                        sw.depthSubImage.imageRect.extent = {
                                static_cast<int32_t>(dsc.width),
                        ...
                        depth_infos[eye].subImage.imageRect.extent = {
                                static_cast<int32_t>(dsc.width),
                                static_cast<int32_t>(dsc.height)};
...
void oxr_interface::create_swapchains() {
    ...
    for (int64_t f : formats) {
        if (f == VK_FORMAT_R8G8B8A8_SRGB)   { chosen_fmt = VK_FORMAT_R8G8B8A8_SRGB;   break; }
        if (f == VK_FORMAT_R8G8B8A8_UNORM)  { chosen_fmt = VK_FORMAT_R8G8B8A8_UNORM;  break; }
        if (f == VK_FORMAT_B8G8R8A8_SRGB)   { chosen_fmt = VK_FORMAT_B8G8R8A8_SRGB;   break; }
    }
    ...
    for (int eye = 0; eye < 2; eye++) {
        ...
        spdlog::get("illixr")->info("Eye {} swapchain: {}x{} format=0x{:X} images={}",
                                    eye, sc.width, sc.height,
    ...
    if (use_depth_ && depth_extension_supported_) {
        ...
        if (spacewarp_supported_) {
            ...
            for (int eye = 0; eye < 2; eye++) {
                ...
                mv_info.usageFlags = XR_SWAPCHAIN_USAGE_SAMPLED_BIT
                                     | XR_SWAPCHAIN_USAGE_COLOR_ATTACHMENT_BIT
                ...
                spdlog::get("illixr")->info(
                        "Eye {} MV swapchain: {}x{} format=0x{:X} images={}",
                        eye, MOTION_VEC_WIDTH, MOTION_VEC_HEIGHT,
                        static_cast<uint32_t>(mv_swapchain_format_), img_count);
            }

            // Spacewarp depth swapchain — D32_SFLOAT (fallback D16_UNORM)
            depth_format_ = VK_FORMAT_D32_SFLOAT;
            for (int64_t f : formats) {
                if (f == VK_FORMAT_D32_SFLOAT) { depth_format_ = VK_FORMAT_D32_SFLOAT; break; }
                if (f == VK_FORMAT_D16_UNORM)  { depth_format_ = VK_FORMAT_D16_UNORM;  break; }
            }
            ...
            for (int eye = 0; eye < 2; eye++) {
                ...
                spdlog::get("illixr")->info(
                        "Eye {} spacewarp depth swapchain: {}x{} format=0x{:X} images={}",
                        eye, MOTION_VEC_WIDTH, MOTION_VEC_HEIGHT,
            ...
            for (int eye = 0; eye < 2; eye++) {
                ...
                spdlog::get("illixr")->info("Eye {} depth swapchain: {}x{} format=0x{:X} images={}",
                                            eye, sc.width, sc.height,
                                            static_cast<uint32_t>(depth_format_), img_count);
            }
        }
