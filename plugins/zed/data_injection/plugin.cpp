...
void data_injection::read_poses() {
    ...
    while (data) {
        ...
        _poses[t] = new pose_data(Eigen::Vector3f{tx, ty, tx}, Eigen::Quaternionf{w, x, y, z}, units::MILLIMETER,
...
void data_injection::_p_one_iteration() {
    ...
    _frame_img_writer.put(_frame_img_writer.allocate<binocular_cam_type>(
        binocular_cam_type{time_point{_clock_duration(_timepoints[_current] + _offset)}, _images.at(image::LEFT_EYE),
                           _images.at(image::RIGHT_EYE)}));
    _frame_pose_writer.put(_frame_pose_writer.allocate<pose_type>(
        pose_type{time_point{_clock_duration(_timepoints[_current] + _offset)}, *_poses.at(_timepoints[_current])}));
    _current++;
