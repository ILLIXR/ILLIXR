...
void capture::get_config() {
    ...
    cam_of << "#width,height,fps,baseline,Lcenter_x,Lcenter_y,Lvfov,Lhfox,Rcenter_x,Rcenter_y,Rvfov,Lhfov" << std::endl;
    cam_of << cc << std::endl;
    cam_of.close();
}
...
void capture::make_gui() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, raw_img[i].cols, raw_img[i].rows, 0, GL_RGB, GL_UNSIGNED_BYTE,
                     raw_img[i].ptr());
    }
