...
void ada_video_encoder::enqueue(cv::Mat& img0, cv::Mat& img1) {
    ...
    GST_BUFFER_PTS(b0) = pts; GST_BUFFER_DURATION(b0) = dur;
    GST_BUFFER_PTS(b1) = pts; GST_BUFFER_DURATION(b1) = dur;
...
GstFlowReturn ada_video_encoder::cb_appsink_msb(GstElement *sink) {
    ...
    if (img1_ready_) {
        ...
        gst_sample_unref(samp0_); samp0_ = nullptr;
        gst_sample_unref(samp1_); samp1_ = nullptr;
