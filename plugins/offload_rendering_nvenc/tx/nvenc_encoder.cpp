...
[[maybe_unused]] nvenc_encoder::nvenc_encoder(int w, int h, int bitrate)
    ...
    //spdlog::get("illixr")->info("HEVC encoder: input {}x{}, aligned {}x{}", width_, height_, aligned_width_, aligned_height_);
    ...
    //spdlog::get("illixr")->info("HEVC encoder initialized: {}x{} @ {} bps", width_, height_, bitrate_);
}
...
void nvenc_encoder::log_hevc_profile() {
    ...
    for (size_t i = 0; i < vps_sps_pps_.size() - 5; i++) {
        if (vps_sps_pps_[i] == 0 && vps_sps_pps_[i + 1] == 0 && vps_sps_pps_[i + 2] == 0 && vps_sps_pps_[i + 3] == 1) {
            ...
            if (nal_type == 33) { // SPS
                ...
                if (i + 10 < vps_sps_pps_.size()) {
                    //spdlog::get("illixr")->info("SPS bytes: {:02X} {:02X} {:02X} {:02X} {:02X} {:02X}", vps_sps_pps_[i + 4],
                    //                            vps_sps_pps_[i + 5], vps_sps_pps_[i + 6], vps_sps_pps_[i + 7],
                    //                            vps_sps_pps_[i + 8], vps_sps_pps_[i + 9]);
                }
            }
        }
    }
...
std::vector<uint8_t> nvenc_encoder::encode(const cv::Mat& frame) {
    ...
    // Convert BGR to NV12
    convert_bgr_to_nv12(frame_to_encode, static_cast<uint8_t*>(lock_input_buffer.bufferDataPtr), lock_input_buffer.pitch);
