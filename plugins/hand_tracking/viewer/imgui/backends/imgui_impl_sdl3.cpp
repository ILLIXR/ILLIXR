...
//  2024-06-26: Update for SDL3 api changes: SDL_StartTextInput()/SDL_StopTextInput()/SDL_SetTextInputRect() functions
//  signatures. 2024-06-24: Update for SDL3 api changes: SDL_EVENT_KEY_DOWN/SDL_EVENT_KEY_UP contents. 2024-06-03; Update for
//  SDL3 api changes: SDL_SYSTEM_CURSOR_ renames. 2024-05-15: Update for SDL3 api changes: SDLK_ renames. 2024-04-15: Inputs:
//  Re-enable calling SDL_StartTextInput()/SDL_StopTextInput() as SDL3 no longer enables it by default and should play nicer
//  with IME. 2024-02-13: Inputs: Fixed gamepad support. Handle gamepad disconnection. Added ImGui_ImplSDL3_SetGamepadMode().
...
//  returning a monotonically increasing value. (#6189, #6114, #3644) 2023-02-07: Forked "imgui_impl_sdl2" into
//  "imgui_impl_sdl3". Removed version checks for old feature. Refer to imgui_impl_sdl2.cpp for older changelog.
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event) {
    ...
    case SDL_EVENT_MOUSE_BUTTON_UP: {
        ...
        bd->MouseButtonsDown = (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN) ? (bd->MouseButtonsDown | (1 << mouse_button))
    ...
    case SDL_EVENT_KEY_UP: {
        // IMGUI_DEBUG_LOG("SDL_EVENT_KEY_%d: key=%d, scancode=%d, mod=%X\n", (event->type == SDL_EVENT_KEY_DOWN) ? "DOWN" :
        // "UP", event->key.key, event->key.scancode, event->key.mod);
        ImGui_ImplSDL3_UpdateKeyModifiers((SDL_Keymod) event->key.mod);
        ImGuiKey key = ImGui_ImplSDL3_KeyEventToImGuiKey(event->key.key, event->key.scancode);
        io.AddKeyEvent(key, (event->type == SDL_EVENT_KEY_DOWN));
        io.SetKeyEventNativeData(key, event->key.key, event->key.scancode,
                                 event->key.scancode); // To support legacy indexing (<1.87 user code). Legacy backend uses
...
static void ImGui_ImplSDL3_UpdateMouseData() {
    ...
    if (is_app_focused) {
        ...
        // (Optional) Fallback to provide mouse position when focused (SDL_EVENT_MOUSE_MOTION already provides this when hovered
        // or captured)
        if (bd->MouseCanUseGlobalState && bd->MouseButtonsDown == 0) {
            // Single-viewport mode: mouse position in client window coordinates (io.MousePos is (0,0) when the mouse is on the
            // upper-left corner of the app window)
...
static void ImGui_ImplSDL3_UpdateGamepadAnalog(ImGui_ImplSDL3_Data* bd, ImGuiIO& io, ImGuiKey key, SDL_GamepadAxis axis_no,
                                               float v0, float v1) {
