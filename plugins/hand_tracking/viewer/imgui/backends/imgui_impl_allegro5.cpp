...
//  functions e.g. ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy ALLEGRO_KEY_* values will also be supported unless
//  IMGUI_DISABLE_OBSOLETE_KEYIO is set] [X] Platform: Clipboard support (from Allegro 5.1.12) [X] Platform: Mouse cursor shape
...
//  io.AddMouseButtonEvent(), io.AddMouseWheelEvent() API (1.87+). 2022-01-17: Inputs: always calling io.AddKeyModsEvent() next
//  and before key event (not in NewFrame) to fix input queue with very low framerates. 2022-01-10: Inputs: calling new
//  io.AddKeyEvent(), io.AddKeyModsEvent() + io.SetKeyEventNativeData() API (1.87+). Support for full ImGuiKey range.
//  2021-12-08: Renderer: Fixed mishandling of the ImDrawCmd::IdxOffset field! This is an old bug but it never had an effect
//  until some internal rendering changes in 1.86. 2021-08-17: Calling io.AddFocusEvent() on
//  ALLEGRO_EVENT_DISPLAY_SWITCH_OUT/ALLEGRO_EVENT_DISPLAY_SWITCH_IN events. 2021-06-29: Reorganized backend to pull data from a
//  single structure to facilitate usage with multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19: Renderer:
//  Replaced direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18:
//  Change blending equation to preserve alpha in output buffer. 2020-08-10: Inputs: Fixed horizontal mouse wheel direction.
...
//  rectangle. 2018-06-13: Renderer: Stopped using al_draw_indexed_prim() as it is buggy in Allegro's DX9 backend. 2018-06-13:
//  Renderer: Backup/restore transform and clipping rectangle. 2018-06-11: Misc: Setup io.BackendFlags
//  ImGuiBackendFlags_HasMouseCursors flag + honor ImGuiConfigFlags_NoMouseCursorChange flag. 2018-04-18: Misc: Renamed file
//  from imgui_impl_a5.cpp to imgui_impl_allegro5.cpp. 2018-04-18: Misc: Added support for 32-bit vertex indices to avoid
//  conversion at runtime. Added imconfig_allegro5.h to enforce 32-bit indices when included from imgui.h. 2018-02-16: Misc:
//  Obsoleted the io.RenderDrawListsFn callback and exposed ImGui_ImplAllegro5_RenderDrawData() in the .h file so you can call
//  it yourself. 2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not available from 1.60 WIP, user needs to call
...
void ImGui_ImplAllegro5_RenderDrawData(ImDrawData* draw_data) {
    ...
    #else
        // Allegro's implementation of al_draw_indexed_prim() for DX9 was broken until 5.2.5. Unindex buffers ourselves while
        // converting vertex format.
    ...
    #if ALLEGRO_HAS_DRAW_INDEXED_PRIM
                al_draw_indexed_prim(&vertices[0], bd->VertexDecl, texture, &indices[pcmd->IdxOffset], pcmd->ElemCount,
...
bool ImGui_ImplAllegro5_Init(ALLEGRO_DISPLAY* display) {
    ...
    // We still use a custom declaration to use 'ALLEGRO_PRIM_TEX_COORD' instead of 'ALLEGRO_PRIM_TEX_COORD_PIXEL' else we can't
    // do a reliable conversion.
