...
private:
	...
	void BuildTimewarp(HMD::hmd_info_t* hmdInfo){
		...
		for(int eye = 0; eye < HMD::NUM_EYES; eye++){
			...
			flat_uv = 		 (HMD::uv_coord_t *) malloc(HMD::NUM_EYES * num_distortion_vertices * sizeof(HMD::uv_coord_t));
		}
		...
		for ( int eye = 0; eye < HMD::NUM_EYES; eye++ )
		{
			for ( int y = 0; y <= hmdInfo->eyeTilesHigh; y++ )
			{
				for ( int x = 0; x <= hmdInfo->eyeTilesWide; x++ )
				{
					...
					flat_uv[eye * num_distortion_vertices + index].u = ( (float)x / hmdInfo->eyeTilesWide );
					flat_uv[eye * num_distortion_vertices + index].v = ( ( ( hmdInfo->eyeTilesHigh - (float)y ) / hmdInfo->eyeTilesHigh ) *
														( (float)( hmdInfo->eyeTilesHigh * hmdInfo->tilePixelsHigh ) / hmdInfo->displayPixelsHigh ) );
...
public:
	...
	virtual void _p_thread_setup() override {
    	...
    	timewarpShaderProgram = init_and_link(meshWarpVertexProgramGLSL, meshWarpFragmentProgramGLSL);
		...
		flat_uv_attr = glGetAttribLocation(timewarpShaderProgram, "flatUv");
		...
		u_near_clip = glGetUniformLocation(timewarpShaderProgram, "u_NearClip");
		u_far_clip = glGetUniformLocation(timewarpShaderProgram, "u_FarClip");

		u_render_inverse_p = glGetUniformLocation(timewarpShaderProgram, "u_renderInverseP");
		u_render_inverse_v = glGetUniformLocation(timewarpShaderProgram, "u_renderInverseV");

		u_warp_vp = glGetUniformLocation(timewarpShaderProgram, "u_warpVP");

		depth_sampler = glGetUniformLocation(timewarpShaderProgram, "_Depth");
