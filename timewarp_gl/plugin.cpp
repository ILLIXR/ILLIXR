...
private:
    ...
    time_type GetNextSwapTimeEstimate() { return time_last_swap + vsync_period; }
...
public:
    ...
    virtual void warp([[maybe_unused]] time_type time)
    {
        ...
        if (log_count > LOG_PERIOD) {
            const double time_since_render_ms_d
                = std::chrono::duration_cast<std::chrono::milliseconds>(time_since_render).count();
        ...
        _m_vsync_estimate.put(_m_vsync_estimate.allocate<switchboard::event_wrapper<time_type>>(GetNextSwapTimeEstimate()));
        ...
        if (enable_offload) {
            ...
            _m_offload_data.put(_m_offload_data.allocate<texture_pose>(
                texture_pose { static_cast<int>(++_offload_seq), /// TODO: Should texture_pose.seq be a long long too?
                               offload_time, image, time_last_swap, latest_pose.pose.position, latest_pose.pose.orientation,
                               most_recent_frame->render_pose.pose.orientation }));
        }

#ifndef NDEBUG
        if (log_count > LOG_PERIOD) {
            const double time_swap
                = std::chrono::duration_cast<std::chrono::milliseconds>(time_after_swap - time_before_swap).count();
            ...
            const double    timewarp_estimate
                = std::chrono::duration_cast<std::chrono::milliseconds>(time_next_swap - time_last_swap).count();
        ...
        timewarp_gpu_logger.log(
            record { timewarp_gpu_record,
                     {
                         { iteration_no },
