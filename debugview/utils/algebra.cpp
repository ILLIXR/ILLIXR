...
void ksAlgebra::ksVector3f_Set( ksVector3f * v, const float value )
{
    v->x = value;
    v->y = value;
    v->z = value;
...
void ksAlgebra::ksVector3f_Add( ksVector3f * result, const ksVector3f * a, const ksVector3f * b )
{
    result->x = a->x + b->x;
    result->y = a->y + b->y;
    result->z = a->z + b->z;
...
void ksAlgebra::ksVector3f_Sub( ksVector3f * result, const ksVector3f * a, const ksVector3f * b )
{
    result->x = a->x - b->x;
    result->y = a->y - b->y;
    result->z = a->z - b->z;
...
void ksAlgebra::ksVector3f_Min( ksVector3f * result, const ksVector3f * a, const ksVector3f * b )
{
...
void ksAlgebra::ksVector3f_Max( ksVector3f * result, const ksVector3f * a, const ksVector3f * b )
{
    result->x = ( a->x > b->x ) ? a->x : b->x;
    result->y = ( a->y > b->y ) ? a->y : b->y;
    result->z = ( a->z > b->z ) ? a->z : b->z;
...
void ksAlgebra::ksVector3f_Decay( ksVector3f * result, const ksVector3f * a, const float value )
{
...
void ksAlgebra::ksVector3f_Lerp( ksVector3f * result, const ksVector3f * a, const ksVector3f * b, const float fraction )
{
...
void ksAlgebra::ksVector3f_Normalize( ksVector3f * v )
{
...
float ksAlgebra::ksVector3f_Length( const ksVector3f * v )
{
    return sqrtf( v->x * v->x + v->y * v->y + v->z * v->z );
}

void ksAlgebra::ksQuatf_Lerp( ksQuatf * result, const ksQuatf * a, const ksQuatf * b, const float fraction )
{
...
void ksAlgebra::ksMatrix3x3f_CreateTransposeFromMatrix4x4f( ksMatrix3x3f * result, const ksMatrix4x4f * src )
{
...
void ksAlgebra::ksMatrix3x4f_CreateFromMatrix4x4f( ksMatrix3x4f * result, const ksMatrix4x4f * src )
{
    result->m[0][0] = src->m[0][0];
    result->m[0][1] = src->m[1][0];
    result->m[0][2] = src->m[2][0];
...
void ksAlgebra::ksMatrix4x4f_Multiply( ksMatrix4x4f * result, const ksMatrix4x4f * a, const ksMatrix4x4f * b )
{
...
void ksAlgebra::ksMatrix4x4f_Transpose( ksMatrix4x4f * result, const ksMatrix4x4f * src )
{
    result->m[0][0] = src->m[0][0];
    result->m[0][1] = src->m[1][0];
    result->m[0][2] = src->m[2][0];
...
float ksAlgebra::ksMatrix4x4f_Minor( const ksMatrix4x4f * matrix, int r0, int r1, int r2, int c0, int c1, int c2 )
{
...
void ksAlgebra::ksMatrix4x4f_Invert( ksMatrix4x4f * result, const ksMatrix4x4f * src )
{
...
void ksAlgebra::ksMatrix4x4f_InvertHomogeneous( ksMatrix4x4f * result, const ksMatrix4x4f * src )
{
    result->m[0][0] = src->m[0][0];
    result->m[0][1] = src->m[1][0];
    result->m[0][2] = src->m[2][0];
...
void ksAlgebra::ksMatrix4x4f_CreateIdentity( ksMatrix4x4f * result )
{
...
void ksAlgebra::ksMatrix4x4f_CreateTranslation( ksMatrix4x4f * result, const float x, const float y, const float z )
{
    result->m[0][0] = 1.0f; result->m[0][1] = 0.0f; result->m[0][2] = 0.0f; result->m[0][3] = 0.0f;
    result->m[1][0] = 0.0f; result->m[1][1] = 1.0f; result->m[1][2] = 0.0f; result->m[1][3] = 0.0f;
    result->m[2][0] = 0.0f; result->m[2][1] = 0.0f; result->m[2][2] = 1.0f; result->m[2][3] = 0.0f;
...
void ksAlgebra::ksMatrix4x4f_CreateRotation( ksMatrix4x4f * result, const float degreesX, const float degreesY, const float degreesZ )
{
...
void ksAlgebra::ksMatrix4x4f_CreateScale( ksMatrix4x4f * result, const float x, const float y, const float z )
{
...
void ksAlgebra::ksMatrix4x4f_CreateFromQuaternion( ksMatrix4x4f * result, const ksQuatf * quat )
{
...
void ksAlgebra::ksMatrix4x4f_CreateTranslationRotationScale( ksMatrix4x4f * result, const ksVector3f * translation, const ksQuatf * rotation, const ksVector3f * scale )
{
...
void ksAlgebra::ksMatrix4x4f_CreateProjection( ksMatrix4x4f * result, const float tanAngleLeft, const float tanAngleRight,
...
void ksAlgebra::ksMatrix4x4f_CreateProjectionFov( ksMatrix4x4f * result, const float fovDegreesLeft, const float fovDegreesRight,
...
void ksAlgebra::ksMatrix4x4f_CreateOffsetScaleForBounds( ksMatrix4x4f * result, const ksMatrix4x4f * matrix, const ksVector3f * mins, const ksVector3f * maxs )
{
...
bool ksAlgebra::ksMatrix4x4f_IsAffine( const ksMatrix4x4f * matrix, const float epsilon )
{
...
bool ksAlgebra::ksMatrix4x4f_IsOrthogonal( const ksMatrix4x4f * matrix, const float epsilon )
{
    for ( int i = 0; i < 3; i++ )
    {
        for ( int j = 0; j < 3; j++ )
...
bool ksAlgebra::ksMatrix4x4f_IsOrthonormal( const ksMatrix4x4f * matrix, const float epsilon )
{
    for ( int i = 0; i < 3; i++ )
    {
        for ( int j = 0; j < 3; j++ )
...
bool ksAlgebra::ksMatrix4x4f_IsHomogeneous( const ksMatrix4x4f * matrix, const float epsilon )
{
...
void ksAlgebra::ksMatrix4x4f_GetTranslation( ksVector3f * result, const ksMatrix4x4f * src )
{
...
void ksAlgebra::ksMatrix4x4f_GetRotation( ksQuatf * result, const ksMatrix4x4f * src )
{
    assert( ksMatrix4x4f_IsAffine( src, 1e-4f ) );
    assert( ksMatrix4x4f_IsOrthogonal( src, 1e-4f ) );

    const float rcpScaleX = RcpSqrt( src->m[0][0] * src->m[0][0] + src->m[0][1] * src->m[0][1] + src->m[0][2] * src->m[0][2] );
    const float rcpScaleY = RcpSqrt( src->m[1][0] * src->m[1][0] + src->m[1][1] * src->m[1][1] + src->m[1][2] * src->m[1][2] );
    const float rcpScaleZ = RcpSqrt( src->m[2][0] * src->m[2][0] + src->m[2][1] * src->m[2][1] + src->m[2][2] * src->m[2][2] );
    const float m[9] =
    {
...
void ksAlgebra::ksMatrix4x4f_GetScale( ksVector3f * result, const ksMatrix4x4f * src )
{
    assert( ksMatrix4x4f_IsAffine( src, 1e-4f ) );
    assert( ksMatrix4x4f_IsOrthogonal( src, 1e-4f ) );
...
void ksAlgebra::ksMatrix4x4f_TransformVector3f( ksVector3f * result, const ksMatrix4x4f * m, const ksVector3f * v )
{
...
void ksAlgebra::ksMatrix4x4f_TransformVector4f( ksVector4f * result, const ksMatrix4x4f * m, const ksVector4f * v )
{
...
void ksAlgebra::ksMatrix4x4f_TransformBounds( ksVector3f * resultMins, ksVector3f * resultMaxs, const ksMatrix4x4f * matrix, const ksVector3f * mins, const ksVector3f * maxs )
{
...
bool ksAlgebra::ksMatrix4x4f_CullBounds( const ksMatrix4x4f * mvp, const ksVector3f * mins, const ksVector3f * maxs )
{
