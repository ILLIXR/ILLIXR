...
// - We never use any of the helpers/facilities used internally by Dear ImGui, unless it has been exposed in the public API
// (imgui.h).
// - We never use maths operators on ImVec2/ImVec4. For other of our sources files, they are provided by imgui_internal.h w/
// IMGUI_DEFINE_MATH_OPERATORS.
//   For your own sources file they are optional and require you either enable those, either provide your own via
//   IM_VEC2_CLASS_EXTRA in imconfig.h. Because we don't want to assume anything about your support of maths operators, we don't
//   use them in imgui_demo.cpp.
...
#pragma clang diagnostic ignored                                                                                               \
    "-Wdeprecated-declarations" // warning : 'xx' is deprecated: The POSIX name for this item.. // for strdup used in demo code
...
#pragma clang diagnostic ignored                                                                                               \
    "-Wunused-macros" // warning : warning: macro is not used                         // we define snprintf/vsnprintf on Windows
                      // so they are available, but not always used.
...
// We split the contents of the big ShowDemoWindow() function into smaller functions (because the link time of very large
// functions grow non-linearly)
...
// You may execute this function to experiment with the UI and understand what it does. You may then search for keywords in the
// code when you are interested by a specific feature.
void ImGui::ShowDemoWindow(bool* p_open)
{
    IM_ASSERT(ImGui::GetCurrentContext() != NULL
              && "Missing dear imgui context. Refer to examples app!"); // Exceptionally add an extra assert here for people
                                                                        // confused with initial dear imgui setup
    ...
    // We specify a default position/size in case there's no data in the .ini file. Typically this isn't required! We only do it
    // to make the Demo applications a little more welcoming.
    ...
    // (default)
    ImGui::PushItemWidth(ImGui::GetFontSize() * -12); // Use fixed width for labels (by passing a negative value), the rest goes
                                                      // to widgets. We choose a width proportional to our font size.
    ...
    if (ImGui::CollapsingHeader("Configuration")) {
        ...
        if (ImGui::TreeNode("Configuration##2")) {
            ...
            ImGui::SameLine();
            HelpMarker("Instruct Dear ImGui to render a mouse cursor for you. Note that a mouse cursor rendered via your "
        ...
        if (ImGui::TreeNode("Backend Flags")) {
            HelpMarker("Those flags are set by the back-ends (imgui_impl_xxx files) to specify their capabilities.\nHere we "
                       "expose then as read-only fields to avoid breaking interactions with your back-end.");
        ...
        if (ImGui::TreeNode("Capture/Logging")) {
            ImGui::TextWrapped("The logging API redirects all text output so you can easily capture the content of a window or "
                               "a block. Tree nodes can be automatically expanded.");
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Basic")) {
...

        {
            ...
            ImGui::SameLine();
            HelpMarker("Refer to the \"Combo\" section below for an explanation of the full BeginCombo/EndCombo API, and "
...

        {
            ...
            HelpMarker("USER:\nHold SHIFT or use mouse to select text.\n"
                       ...
                       "need to wire InputText() to a dynamic string type. See misc/cpp/imgui_stdlib.h for an example (this is "
                       "not demonstrated in imgui_demo.cpp).");
...

        {
            ...
            HelpMarker("Click on the colored square to open a color picker.\nClick and hold to use drag and drop.\nRight-click "
                       "on the colored square to show options.\nCTRL+click on individual component to input value.\n");
    ...
    if (ImGui::TreeNode("Trees")) {
        ...
        if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
            ...
            ImGui::SameLine();
            HelpMarker("Extend hit area to all available width instead of allowing more items to be layed out after the node.");
            ...
            static int selection_mask
                = (1 << 2); // Dumb representation of what may be user-side selection state. You may carry selection state
                            // inside or outside your objects in whatever format you see fit.
            int node_clicked = -1; // Temporary storage of what node we have clicked to process selection at the end of the
                                   // loop. May be a pointer to your own node type, etc.
            for (int i = 0; i < 6; i++) {
            ...
            if (node_clicked != -1) {
                ...
                else // if (!(selection_mask & (1 << node_clicked))) // Depending on selection behavior you want, this commented
                     // bit preserve selection when clicking on item that is part of the selection
    ...
    if (ImGui::TreeNode("Text")) {
        ...
        if (ImGui::TreeNode("Word Wrapping")) {
            ...
            ImGui::TextWrapped("This text should automatically wrap on the edge of the window. The current implementation for "
                               "text wrapping follows simple rules suitable for English and possibly other languages.");
            ImGui::Spacing();
            ...
            ImGui::Text("The lazy dog is a good dog. This paragraph is made to fit within %.0f pixels. Testing a 1 character "
        ...
        if (ImGui::TreeNode("UTF-8 Text")) {
            ...
            // - For earlier compiler, you may be able to encode your sources as UTF-8 (e.g. Visual Studio save your file as
            // 'UTF-8 without signature')
            // - FOR THIS DEMO FILE ONLY, BECAUSE WE WANT TO SUPPORT OLD COMPILERS, WE ARE *NOT* INCLUDING RAW UTF-8 CHARACTERS
            // IN THIS SOURCE FILE.
            //   Instead we are encoding a few strings with hexadecimal constants. Don't do this in your application!
            //   Please use u8"text in any language" in your application!
            // Note that characters values are preserved even by InputText() if the font cannot be displayed, so you can safely
            // copy & paste garbled characters into another application.
            ImGui::TextWrapped(
                "CJK text will only appears if the font was loaded with the appropriate CJK character ranges. Call "
                "io.Font->AddFontFromFileTTF() manually to load extra character ranges. Read docs/FONTS.txt for details.");
            ImGui::Text(
    ...
    if (ImGui::TreeNode("Images")) {
        ImGuiIO& io = ImGui::GetIO();
        ImGui::TextWrapped("Below we are displaying the font texture (which is the only texture we have access to in this "
                           "demo). Use the 'ImTextureID' type as storage to pass pointers or identifier to your own texture "
                           "data. Hover the texture for a zoomed view!");

        // Here we are grabbing the font texture because that's the only one we have access to inside the demo code.
        // Remember that ImTextureID is just storage for whatever you want it to be, it is essentially a value that will be
        // passed to the render function inside the ImDrawCmd structure. If you use one of the default imgui_impl_XXXX.cpp
        // renderer, they all have comments at the top of their file to specify what they expect to be stored in ImTextureID.
        // (for example, the imgui_impl_dx11.cpp renderer expect a 'ID3D11ShaderResourceView*' pointer. The
        // imgui_impl_opengl3.cpp renderer expect a GLuint OpenGL texture identifier etc.) If you decided that ImTextureID =
        // MyEngineTexture*, then you can pass your MyEngineTexture* pointers to ImGui::Image(), and gather width/height through
        // your own functions, etc. Using ShowMetricsWindow() as a "debugger" to inspect the draw data that are being passed to
        // your render will help you debug issues if you are confused about this. Consider using the lower-level
        ...
        if (ImGui::IsItemHovered()) {
            ...
            ImGui::Image(my_tex_id, ImVec2(region_sz * zoom, region_sz * zoom), uv0, uv1, ImVec4(1.0f, 1.0f, 1.0f, 1.0f),
                         ImVec4(1.0f, 1.0f, 1.0f, 0.5f));
            ImGui::EndTooltip();
        }
    ...
    if (ImGui::TreeNode("Combo")) {
        ...
        // (your selection data could be an index, a pointer to the object, an id for the object, a flag stored in the object
        // itself, etc.)
        const char*        items[]      = { "AAAA", "BBBB", "CCCC", "DDDD", "EEEE",    "FFFF", "GGGG",
                                "HHHH", "IIII", "JJJJ", "KKKK", "LLLLLLL", "MMMM", "OOOOOOO" };
    ...
    if (ImGui::TreeNode("Selectables")) {
        ...
        // - The one taking "bool selected" as a read-only selection information. When Selectable() has been clicked is returns
        // true and you can alter selection state accordingly.
        // - The one taking "bool* p_selected" as a read-write selection information (convenient in some cases)
        // The earlier is more flexible, as in real application your selection may be stored in a different manner (in flags
        ...
        if (ImGui::TreeNode("Grid")) {
            ...
            for (int i = 0; i < 4 * 4; i++) {
                ...
                if (ImGui::Selectable("Sailor", &selected[i], 0, ImVec2(50, 50))) {
                    // Note: We _unnecessarily_ test for both x/y and i here only to silence some static analyzer. The second
                    // part of each test is unnecessary.
                    int x = i % 4;
                    int y = i / 4;
                    if (x > 0) {
                        selected[i - 1] ^= 1;
        ...
        if (ImGui::TreeNode("Alignment")) {
            HelpMarker("By default, Selectables uses style.SelectableTextAlign but it can be overriden on a per-item basis "
                       "using PushStyleVar(). You'll probably want to always keep your default situation to left-align "
                       "otherwise it becomes difficult to layout multiple items on a same line");
    ...
    if (ImGui::TreeNode("Text Input")) {
        ...
        if (ImGui::TreeNode("Resize Callback")) {
            // To wire InputText() with std::string or any other custom string type,
            // you can use the ImGuiInputTextFlags_CallbackResize flag + create a custom ImGui::InputText() wrapper using your
            // prefered type. See misc/cpp/imgui_stdlib.h for an implementation of this using std::string.
            HelpMarker("Demonstrate using ImGuiInputTextFlags_CallbackResize to wire your resizable string type to "
            ...
            struct Funcs
            {
                ...
                // Tip: Because ImGui:: is a namespace you would typicall add your own function into the namespace in your own
                // source files. For example, you may add a function called ImGui::InputText(const char* label, MyString*
                // my_str).
            ...
            // Note that because we need to store a terminating zero character, our size/capacity are 1 more than usually
            // reported by a typical string class.
    ...
    if (ImGui::TreeNode("Plots Widgets")) {
        ...
        // Tip: If your float aren't contiguous but part of a structure, you can pass a pointer to your first float and the
        // sizeof() of your structure in the Stride parameter.
        ...
        // FIXME: This is rather awkward because current plot API only pass in indices. We probably want an API passing floats
        // and user provide sample rate/count.
        struct Funcs
        {
    ...
    if (ImGui::TreeNode("Color/Picker Widgets")) {
        ...
        ImGui::SameLine();
        HelpMarker("ColorEdit defaults to displaying RGB inputs if you don't specify a display mode, but the user can change "
        ...
        ImGui::SameLine();
        HelpMarker("SetColorEditOptions() is designed to allow you to set boot-time default.\nWe don't have Push/Pop functions "
                   "because you can force options on a per-widget basis if needed, and the user can change non-forced ones "
                   "with the options menu.\nWe don't have a getter to avoid encouraging you to persistently save values that "
        ...
        ImGui::SameLine();
        HelpMarker("By default, colors are given to ColorEdit and ColorPicker in RGB, but ImGuiColorEditFlags_InputHSV allows "
...
// Note that the SliderScalar function has a maximum usable range of half the natural type maximum, hence the /2 below.
    ...
    if (ImGui::TreeNode("Drag and Drop")) {
        if (ImGui::TreeNode("Drag and drop in standard widgets")) {
            ...
            // They are using standardized payload strings IMGUI_PAYLOAD_TYPE_COLOR_3F and IMGUI_PAYLOAD_TYPE_COLOR_4F to allow
            // your own widgets to use colors in their drag and drop interaction. Also see the demo in Color Picker -> Palette
            // demo.
        ...
        if (ImGui::TreeNode("Drag and drop to copy/swap items")) {
            ...
            for (int n = 0; n < IM_ARRAYSIZE(names); n++) {
                ...
                if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
                    ...
                    } // Display preview (could be anything, e.g. when dragging an image we could decide to display the filename
                      // and a small preview of the image, etc.)
                    if (mode == Mode_Move) {
        ...
        if (ImGui::TreeNode("Drag to reorder items (simple)")) {
            // Simple reordering
            HelpMarker("We don't use the drag and drop api at all here! Instead we query when the item is held but not "
    ...
    if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
        ...
        ImGui::Combo(
            ...
            "repeat)\0Checkbox\0SliderFloat\0InputText\0InputFloat\0InputFloat3\0ColorEdit4\0MenuItem\0TreeNode\0TreeNode (w/ "
        ...
        ImGui::BulletText("Return value = %d\n"
                          ...
                          ImGui::IsItemVisible(), ImGui::IsItemClicked(), ImGui::IsItemToggledOpen(), ImGui::GetItemRectMin().x,
                          ImGui::GetItemRectMin().y, ImGui::GetItemRectMax().x, ImGui::GetItemRectMax().y,
        ...
        // This is useful in particular if you want to create a context menu (with BeginPopupContextItem) associated to the
...
static void ShowDemoWindowLayout()
{
    ...
    if (ImGui::TreeNode("Child windows")) {
        ...
        // - Using SetCursorPos() to position the child window (because the child window is an item from the POV of the parent
        // window)
        //   You can also call SetNextWindowPos() to position the child window. The parent window will effectively layout from
        //   this position.
        // - Using ImGui::GetItemRectMin/Max() to query the "item" state (because the child window is an item from the POV of
        // the parent window)
    ...
    if (ImGui::TreeNode("Widgets Width")) {
        ...
        // Demonstrate using PushItemWidth to surround three items. Calling SetNextItemWidth() before each of them would have
        // the same effect.
    ...
    if (ImGui::TreeNode("Basic Horizontal Layout")) {
        ...
        // Manually wrapping (we should eventually provide this as an automatic layout feature, but for now you can do it
        // manually)
    ...
    if (ImGui::TreeNode("Tabs")) {
        ...
        if (ImGui::TreeNode("Advanced & Close Button")) {
            ...
            // Passing a bool* to BeginTabItem() is similar to passing one to Begin(): the underlying bool will be set to false
    ...
    if (ImGui::TreeNode("Groups")) {
        HelpMarker(
            "BeginGroup() basically locks the horizontal position for new line. EndGroup() bundles the whole group so that you "
            "can use \"item\" functions such as IsItemHovered()/IsItemActive() or SameLine() etc. on the whole group.");
        ImGui::BeginGroup();
        {
            ImGui::BeginGroup();
            ImGui::Button("AAA");
    ...
    if (ImGui::TreeNode("Text Baseline Alignment")) {
        {
            ...
            ImGui::SameLine();
            HelpMarker(
                "This is testing the vertical alignment that gets applied on text to keep it aligned with widgets. Lines only "
                "composed of text or \"small\" widgets fit in less vertical spaces than lines with normal widgets.");
            ImGui::Indent();
            ...
            // Because we don't know what's coming after the Text() statement, we need to move the text baseline down by
            // FramePadding.y
            ImGui::AlignTextToFramePadding();
...

        {
            ...
            ImGui::AlignTextToFramePadding(); // Vertically align text node a bit lower so it'll be vertically centered with
                                              // upcoming widget. Otherwise you can use SmallButton (smaller fit).
            bool node_open = ImGui::TreeNode("Node##2"); // Common mistake to avoid: if we want to SameLine after TreeNode we
                                                         // need to do it before we add child content.
            ImGui::SameLine(0.0f, spacing);
    ...
    if (ImGui::TreeNode("Scrolling")) {
        ...
        HelpMarker(
            ...
            "Pos\" button above will make the discontinuity at edges visible: scrolling to the top/bottom/left/right-most item "
            "will add an additional WindowPadding to reflect on reaching the edge of the list.\n\nBecause the clipping "
            "rectangle of most window hides half worth of WindowPadding on the left/right, using SetScrollFromPosX(+1) will "
            "usually result in clipped text whereas the equivalent SetScrollFromPosY(+1) wouldn't.");
        ...
        HelpMarker(
            "Horizontal scrolling for a window has to be enabled explicitly via the ImGuiWindowFlags_HorizontalScrollbar "
        ...
        for (int line = 0; line < lines; line++) {
            // Display random stuff (for the sake of this trivial demo we are using basic Button+SameLine. If you want to create
            // your own time line for a real application you may be better off manipulating the cursor position yourself, aka
            // using SetCursorPos/SetCursorScreenPos to position the widgets yourself. You may also want to use the lower-level
            // ImDrawList API)
        ...
        if (scroll_x_delta != 0.0f) {
            ImGui::BeginChild("scrolling"); // Demonstrate a trick: you can use Begin to set yourself in the context of another
    ...
    if (ImGui::TreeNode("Clipping")) {
        ...
        ImGui::TextWrapped("On a per-widget basis we are occasionally clipping text CPU-side if it won't fit in its frame. "
                           "Otherwise we are doing coarser clipping + passing a scissor rectangle to the renderer. The system "
                           "is designed to try minimizing both execution and CPU/GPU rendering cost.");
...
static void ShowDemoWindowPopups()
{
    ...
    // - Their visibility state (~bool) is held internally by Dear ImGui instead of being held by the programmer as we are used
    ...
    if (ImGui::TreeNode("Popups")) {
        ImGui::TextWrapped("When a popup is active, it inhibits interacting with windows that are behind the popup. Clicking "
    ...
    if (ImGui::TreeNode("Context menus")) {
        ...
        // So here we will make it that clicking on the text field with the right mouse button (1) will toggle the visibility of
        // the popup above.
        ...
        // In addition here, we want to include your editable label inside the button label. We use the ### operator to override
        // the ID (read FAQ about ID for details)
    ...
    if (ImGui::TreeNode("Modals")) {
        ...
        if (ImGui::BeginPopupModal("Stacked 1", NULL, ImGuiWindowFlags_MenuBar)) {
            ...
            // Also demonstrate passing a bool* to BeginPopupModal(), this will create a regular close button which will close
            // the popup. Note that the visibility state of popups is owned by imgui, so the input value of the bool actually
            // doesn't matter here.
    ...
    if (ImGui::TreeNode("Menus inside a regular window")) {
        ...
        ImGui::Separator();
        // NB: As a quirk in this very specific example, we want to differentiate the parent of this menu from the parent of the
        // various popup menus above. To do so we are encloding the items in a PushID()/PopID() block to make them two different
        // menusets. If we don't, opening any popup above and hovering our menu here would open it. This is because once a menu
        // is active, we allow to switch to a sibling menu by just hovering on it, which is the desired behavior for regular
        // menus.
        ImGui::PushID("foo");
...
static void ShowDemoWindowMisc()
{
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
        ...
        if (ImGui::TreeNode("Dragging")) {
            ...
            // Drag operations gets "unlocked" when the mouse has moved past a certain threshold (the default threshold is
            // stored in io.MouseDragThreshold) You can request a lower or higher threshold using the second parameter of
        ...
        if (ImGui::TreeNode("Mouse cursors")) {
            ...
            ImGui::SameLine();
            HelpMarker("Your application can render a different mouse cursor based on what ImGui::GetMouseCursor() returns. If "
                       "software cursor rendering (io.MouseDrawCursor) is set ImGui will draw the right cursor for you, "
...
// Here we use the simplified Combo() api that packs items into a single literal string. Useful for quick combo boxes where the
...
{
    // You can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it compares to an internally
    // stored reference)
    ImGuiStyle&       style = ImGui::GetStyle();
    ...
    if (ImGui::Button("Revert Ref"))
        style = *ref;
    ImGui::SameLine();
    HelpMarker("Save/Revert in local non-persistent storage. Default Colors definition are not affected. Use \"Export\" below "
        ...
        if (ImGui::BeginTabItem("Colors")) {
            ...
            for (int i = 0; i < ImGuiCol_COUNT; i++) {
                ...
                if (memcmp(&style.Colors[i], &ref->Colors[i], sizeof(ImVec4)) != 0) {
                    ...
                    // instead of "Save"/"Revert" you'd use icons. Read the FAQ and docs/FONTS.txt about using icon fonts. It's
            ...
            for (int i = 0; i < atlas->Fonts.Size; i++) {
                ...
                if (font_details_opened) {
                    ...
                    HelpMarker("Note than the default embedded font is NOT meant to be scaled.\n\nFont are currently rendered "
                               ...
                               "some flexibility with scaling. You can also render at multiple sizes and select which one to "
                    ...
                    ImGui::Text("Texture Area: about %d px ~%dx%d px", font->MetricsTotalSurface, (int)surface_sqrt,
                                (int)surface_sqrt);
                    ...
                    if (ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size)) {
                        ...
                        for (unsigned int base = 0; base <= IM_UNICODE_CODEPOINT_MAX; base += 256) {
                            ...
                            // IM_UNICODE_MAX_CODEPOINT is large. (if ImWchar==ImWchar32 we will do at least about 272 queries
                            // here)
                            ...
                            if (count > 0
                                && ImGui::TreeNode((void*)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base, base + 255, count,
...
struct ExampleAppConsole
{
    ...
    ExampleAppConsole()
    {
        ...
        Commands.push_back("CLASSIFY"); // "classify" is only here to provide an example of "C"+[tab] completing to "CL" and
                                        // displaying matches.
        AutoScroll     = true;
    ...
    void Draw(const char* title, bool* p_open)
    {
        ...
        ImGui::TextWrapped("This example implements a console with basic coloring, completion and history. A more elaborate "
    ...
    int TextEditCallback(ImGuiInputTextCallbackData* data)
    {
        ...
        case ImGuiInputTextFlags_CallbackCompletion: {
            ...
            if (candidates.Size == 0) {
                ...
                // Multiple matches. Complete as much as we can, so inputing "C" will complete to "CL" and display "CLEAR" and
                // "CLASSIFY"
...
struct ExampleAppLog
{
    ImGuiTextBuffer Buf;
    ImGuiTextFilter Filter;
    ImVector<int>   LineOffsets; // Index to lines offset. We maintain this with AddLog() calls, allowing us to have a random
                               // access on lines
    ...
    void Draw(const char* title, bool* p_open = NULL)
    {
        ...
        if (Filter.IsActive()) {
            ...
            // And it'll just work. TextUnformatted() has specialization for large blob of text and will fast-forward to skip
            // non-visible lines. Here we instead demonstrate using the clipper to only process lines that are within the
            // visible area. If you have tens of thousands of items and their processing cost is non-negligible, coarse clipping
            // them on your side is recommended. Using ImGuiListClipper requires A) random access into your data, and B) items
            // all being the  same height, both of which we can handle since we an array pointing to the beginning of each line
            // of text. When using the filter (in the block of code above) we don't have random access into the data to display
            // anymore, which is why we don't use the clipper. Storing or skimming through the search result would make it
            // possible (and would be recommended if you want to search through tens of thousands of entries)
    ...
    if (ImGui::SmallButton("[Debug] Add 5 entries")) {
        ...
        for (int n = 0; n < 5; n++) {
            ...
            log.AddLog("[%05d] [%s] Hello, current time is %.1f, here's a word: '%s'\n", ImGui::GetFrameCount(),
                       categories[counter % IM_ARRAYSIZE(categories)], ImGui::GetTime(), words[counter % IM_ARRAYSIZE(words)]);
            counter++;
        }
    }
    ImGui::End();
...
static void ShowExampleAppPropertyEditor(bool* p_open)
{
    ...
    struct funcs
    {
        static void ShowDummyObject(const char* prefix, int uid)
        {
            ...
            ImGui::AlignTextToFramePadding(); // Text and Tree nodes are less high than regular widgets, here we add vertical
            ...
            if (node_open) {
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    if (i < 2) {
                        ...
                        ImGui::AlignTextToFramePadding();
                        ImGui::TreeNodeEx(
                            "Field", ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen | ImGuiTreeNodeFlags_Bullet,
                            "Field_%d", i);
                        ImGui::NextColumn();
                        ImGui::SetNextItemWidth(-1);
                        if (i >= 5)
...
// This apply to all regular items as well. Read FAQ section "How can I have multiple widgets with the same label? Can I have
// widget without a label? (Yes). A primer on the purpose of labels/IDs." for details.
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    // Tip: If you do a lot of custom rendering, you probably want to use your own geometrical types and benefit of overloaded
    // operators, etc. Define IM_VEC2_CLASS_EXTRA in imconfig.h to create implicit conversions between your types and
    // ImVec2/ImVec4. ImGui defines overloaded operators but they are internal to imgui.cpp and not exposed outside (to avoid
    // messing with your types) In this example we are not using the maths operators!
    ImDrawList* draw_list = ImGui::GetWindowDrawList();
    ...
    if (ImGui::BeginTabBar("##TabBar")) {
        if (ImGui::BeginTabItem("Primitives")) {
            ...
            ImVec2 gradient_size = ImVec2(ImGui::CalcItemWidth(), ImGui::GetFrameHeight());
            {
                ...
                draw_list->AddRectFilledMultiColor(p, ImVec2(p.x + gradient_size.x, p.y + gradient_size.y), col_a, col_b, col_b,
                                                   col_a);
        ...
        if (ImGui::BeginTabItem("Canvas")) {
            ...
            // IsItemHovered() But you can also draw directly and poll mouse/keyboard by yourself. You can manipulate the cursor
            // using GetCursorPos() and SetCursorPos(). If you only use the ImDrawList API, you can notify the owner window of
...
// to clicking on the regular tab closing button) and stops being submitted, it will take a frame for the tab bar to notice its
// absence. During this frame there will be a gap in the tab bar, and if the tab that has disappeared was the selected one, the
// tab bar will report no selected tab during the frame. This will effectively give the impression of a flicker for one frame.
