...
// to imconfig.h) B) or add configuration directives in your own file and compile with #define IMGUI_USER_CONFIG "myfilename.h"
// If you do so you need to make sure that configuration settings are defined consistently _everywhere_ Dear ImGui is used,
// which include the imgui*.cpp files but also _any_ of your code that uses Dear ImGui. This is because some compile-time
// options have an affect on data structures. Defining those options in imconfig.h will ensure every compilation unit gets to
// see the same data structure layouts. Call IMGUI_CHECKVERSION() from your .cpp files to verify that the data structures your
// files are using are matching the ones imgui.cpp is using.
//-----------------------------------------------------------------------------
...
//'-framework ApplicationServices', this is why this is not the default). #define IMGUI_DISABLE_DEFAULT_FORMAT_FUNCTIONS //
//Don't implement ImFormatString/ImFormatStringV so you can implement them yourself (e.g. if you don't want to link with
//vsnprintf) #define IMGUI_DISABLE_DEFAULT_MATH_FUNCTIONS              // Don't implement
...
// compatibility checks of the arguments and formats done by clang and GCC will be disabled in order to support the extra
