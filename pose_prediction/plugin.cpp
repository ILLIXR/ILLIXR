...
public:
    ...
    virtual fast_pose_type get_fast_pose(time_type future_timestamp) const override
    {
        ...
        double dt
            = std::chrono::duration_cast<std::chrono::nanoseconds>(future_timestamp - std::chrono::system_clock::now()).count();
        ...
        return fast_pose_type { predicted_pose, std::chrono::high_resolution_clock::now(), future_timestamp };
    }
