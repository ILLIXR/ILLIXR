...
void egl_context_manager::register_primary_context(EGLDisplay display, EGLContext context, EGLConfig config) {
    ...
    spdlog::get("illixr")->info("egl_context_manager: Primary context registered: {}", (void*) context);
...
EGLContext egl_context_manager::get_context_for_current_thread() {
    ...
    if (new_context != EGL_NO_CONTEXT) {
        ...
        spdlog::get("illixr")->info("egl_context_manager: Created shared context {} for thread", (void*) new_context);
    }
...
bool egl_context_manager::make_current_for_thread() {
    ...
    if (current != EGL_NO_CONTEXT) {
        spdlog::get("illixr")->warn("egl_context_manager: Different context {} was current, expected {}", (void*) current,
                                    (void*) ctx);
    }
...
bool egl_context_manager::wait_for_initialization(int timeout_ms) {
    ...
    return init_cv_.wait_for(lock, std::chrono::milliseconds(timeout_ms), [this] {
...
EGLContext egl_context_manager::create_shared_context() {
    ...
    if (shared == EGL_NO_CONTEXT) {
        EGLint error = eglGetError();
        spdlog::get("illixr")->error("egl_context_manager: Failed to create shared context: 0x{:X}", error);
    }
