...
public:
    pose_lookup_impl(const phonebook* const pb)
        ...
        , _m_vsync_estimate { sb->get_reader<switchboard::event_wrapper<time_type>>("vsync_estimate") } /// TODO: Set with #198
    ...
    {
        const switchboard::ptr<const switchboard::event_wrapper<time_type>> estimated_vsync
    ...
    virtual pose_type get_true_pose() const override { throw std::logic_error { "Not Implemented" }; }

    virtual bool fast_pose_reliable() const override { return true; }

    virtual bool true_pose_reliable() const override { return false; }

    virtual Eigen::Quaternionf get_offset() override { return offset; }
    ...
    virtual fast_pose_type get_fast_pose(time_type time) const override
    {
...
#ifndef NDEBUG
            std::cerr << "Time " << lookup_time << " (" << std::chrono::nanoseconds(time - _m_start_of_time).count() << " + "
        ...
        return fast_pose_type { .pose                  = correct_pose(looked_up_pose),
