...
static std::map<ullong, sensor_types> load_data() {
	...
	spdlog::get("illixr_log")->error("[OFFLINEIMU] Please define ILLIXR_DATA");
        ILLIXR::abort();
    }
    ...
    if (!imu0_file.good()) {
	spdlog::get("illixr_log")->error("[OFFLINEIMU] ${ILLIXR_DATA} {0} ({1}{0}) is not a good path",imu0_subpath,illixr_data);
        ILLIXR::abort();
    }
