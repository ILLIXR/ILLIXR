...
public:
    ...
    gldemo(std::string name_, phonebook* pb_)
        ...
        , sb { pb->lookup_impl<switchboard>() } //, xwin{pb->lookup_impl<xlib_gl_extended_window>()}
    ...
    {
        {
            ...
            if (log_count > LOG_PERIOD) {
                std::cout << "\033[1;32m[GL DEMO APP]\033[0m Submitting frame to buffer " << buffer_to_use
                          << ", frametime: " << time_since_last_d << ", FPS: " << fps << std::endl;
            }
#endif
            ...
            _m_eyebuffer.put(_m_eyebuffer.allocate<rendered_frame>(
                rendered_frame { std::array<GLuint, 2> { eyeTextures[0], eyeTextures[1] }.data(),
                                 std::array<GLuint, 2> { buffer_to_use, buffer_to_use }.data(), fast_pose,
                                 fast_pose_sample_time, std::chrono::system_clock::now() }));
...
private:
    ...
    void createFBO(GLuint* texture_handle, GLuint* fbo, GLuint* depth_target)
    {
        ...
        // glRenderbufferStorageMultisample(GL_RENDERBUFFER, fboSampleCount, GL_DEPTH_COMPONENT, EYE_TEXTURE_WIDTH,
        // EYE_TEXTURE_HEIGHT);
