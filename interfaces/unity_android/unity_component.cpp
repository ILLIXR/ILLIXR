...
void unity_component::send_voice_query(uint64_t query_id_, const uint8_t* pcm_data, int32_t pcm_len, float similarity_threshold,
                                       float min_match_similarity) {
    spdlog::get("illixr")->debug("Starting send for {}, pcm_pointer={}, len={}, threshold={}, similarity={}", query_id_,
                                 static_cast<void*>(const_cast<uint8_t*>(pcm_data)), pcm_len, similarity_threshold,
...
bool unity_component::get_query_response(uint64_t* out_query_id, float* out_centroids, int32_t* out_num_clouds,
                                         float* out_colors, int32_t out_colors_max, float* out_server_latency,
...
extern "C" void illixr_unity_send_voice_query(uint64_t query_id_, const uint8_t* pcm_data, int32_t pcm_len,
                                              float similarity_threshold, float min_match_similarity) {
    if (unity_component_obj == nullptr)
        return;

    unity_component_obj->send_voice_query(query_id_, pcm_data, pcm_len, similarity_threshold, min_match_similarity);
}
...
extern "C" int illixr_unity_get_query_response(uint64_t* out_query_id,
                                               ...
                                               int32_t* out_num_clouds, float* out_colors, int32_t out_colors_max,
    ...
    return unity_component_obj->get_query_response(out_query_id, out_centroids, out_num_clouds, out_colors, out_colors_max,
                                                   out_server_latency, out_text_query, text_query_buf_len)
        ? 1
        : 0;
}

extern "C" int illixr_unity_get_query_response_info(uint64_t* out_query_id, int32_t* out_num_clouds, int32_t* out_total_points,
                                                    int32_t* out_points_per_cloud, int32_t points_per_cloud_max,
                                                    float* out_centroids, float* out_colors, int32_t out_colors_max,
                                                    int32_t* out_num_colors, float* out_server_latency, char* out_text_query,
                                                    int32_t text_query_buf_len) {
    if (unity_component_obj == nullptr)
        return 0;
    return unity_component_obj->get_query_response_info(out_query_id, out_num_clouds, out_total_points, out_points_per_cloud,
                                                        points_per_cloud_max, out_centroids, out_colors, out_colors_max,
                                                        out_num_colors, out_server_latency, out_text_query, text_query_buf_len)
        ? 1
        : 0;
}

extern "C" int illixr_unity_get_query_response_points(uint64_t query_id_, float* out_points, int32_t points_max) {
    if (unity_component_obj == nullptr)
        return 0;
    return unity_component_obj->get_query_response_points(query_id_, out_points, points_max) ? 1 : 0;
}

bool unity_component::get_query_response_info(uint64_t* out_query_id, int32_t* out_num_clouds, int32_t* out_total_points,
                                              int32_t* out_points_per_cloud, int32_t points_per_cloud_max, float* out_centroids,
                                              float* out_colors, int32_t out_colors_max, int32_t* out_num_colors,
                                              float* out_server_latency, char* out_text_query, int32_t text_query_buf_len) {
    auto response = response_reader_.get_ro_nullable();
    if (!response)
        return false;
...
bool unity_component::get_query_response_points(uint64_t query_id_, float* out_points, int32_t points_max) {
    ...
    spdlog::get("illixr")->debug("get_query_response_points: id={} written={} floats", query_id_, written);
