...
#ifndef NDEBUG
		spdlog::get(name)->debug("[REALSENSE] Found Product Line: {}", product_line);
#endif
                if (product_line == "D400") {
#ifndef NDEBUG
		    spdlog::get(name)->debug("[REALSENSE] Checking for supported streams");
#endif
                    ...
                    if (accel_found && gyro_found) {
                        D4XXI_found = true;
#ifndef NDEBUG
			spdlog::get(name)->debug("[REALSENSE] Supported D4XX found!");
#endif
                    }
                } else if (product_line == "T200") {
                    T26X_found = true;
#ifndef NDEBUG
		    spdlog::get(name)->debug("[REALSENSE] T26X found!");
#endif
                }
            }
        }
...
#ifndef NDEBUG
	    spdlog::get(name)->debug("[REALSENSE] No supported Realsense device detected!");
#endif
        }
    }
    ...
    void configure_camera() {
        ...
        if (realsense_cam.compare("auto") == 0) {
            if (D4XXI_found) {
                cam_select = D4XXI;
#ifndef NDEBUG
		spdlog::get(name)->debug("[REALSENSE] Setting cam_select: D4XX");
#endif
            } else if (T26X_found) {
                cam_select = T26X;
#ifndef NDEBUG
		spdlog::get(name)->debug("[REALSENSE] Setting cam_select: T26X");
#endif
            }
