...
namespace detail {
    ...
    public:
        Frame(WallTime process_start_, const char* function_name_, const char* file_name_, size_t line_, size_t index_,
              size_t caller_index_, size_t prev_index_, TypeEraser info_)
         ...
         */
        const TypeEraser& get_info() const {
            return info;
        }
         ...
         */
        WallTime get_stop_wall() const {
            return stop_wall == WallTime{0} ? WallTime{0} : stop_wall - process_start;
        }

        /**
         * @brief See documentation of return type.
         */
        CpuTime get_start_cpu() const {
            return start_cpu;
        }
         ...
         * @brief See documentation of return type.
         */
        WallTime get_start_wall() const {
            return start_wall == WallTime{0} ? WallTime{0} : start_wall - process_start;
        }

        /**
         * @brief See documentation of return type.
         */
        CpuTime get_stop_cpu() const {
            return stop_cpu;
        }
         ...
         */
        size_t get_youngest_callee_index() const {
         ...
         */
        bool is_leaf() const {
            return youngest_child_index == 0;
        }
    };

    CPU_TIMER_UNUSED static std::ostream& operator<<(std::ostream& os, const Frame& frame) {
        return os << "frame[" << frame.get_index() << "] = " << null_to_empty(frame.get_file_name()) << ":" << frame.get_line()
                  << ":" << null_to_empty(frame.get_function_name()) << " called by frame[" << frame.get_caller_index()
                  << "]\n";
    }
    ...
    private:
        ...
        void enter_stack_frame(const char* function_name, const char* file_name, size_t line, TypeEraser info) {
            ...
            stack.emplace_back(get_process_start(), function_name, file_name, line, this_index, caller_index, prev_index,
                               std::move(info));
        ...
        void exit_stack_frame(CPU_TIMER_UNUSED const char* function_name) {
            ...
            assert(function_name == stack.back().get_function_name() &&
                   "somehow enter_stack_frame and exit_stack_frame for this frame are misaligned");
            {
    ...
    public:
        ...
        void record_event(bool wall_time, bool cpu_time, const char* function_name, const char* file_name, size_t line,
                          TypeEraser info) {
            size_t this_index = index++;
            ...
            finished.emplace_back(get_process_start(), function_name, file_name, line, this_index, caller_index, prev_index,
                                  std::move(info));
        ...
        ~Stack() {
            ...
            // assert(finished.empty() && "flush() should drain this buffer, and nobody should be adding to it now. Somehow
            // unflushed Frames are still present");
        }
        ...
        void maybe_flush() {
            ...
            if (!finished.empty()) {
                ...
                if (get_ns(process_log_period) != 0 &&
                    (get_ns(process_log_period) == 1 || now > last_log + process_log_period)) {
     ...
     *
     * This calls callback with one thread's batches of Frames, periodically not sooner than log_period, in the thread whose
     * functions are in the batch.
     */
    class Process {
    private:
        friend class Stack;
    ...
    public:
         ...
         */
        Stack& create_stack(std::thread::id thread, std::thread::native_handle_type native_handle, std::string&& thread_name) {
            ...
            if (thread_to_stack.count(thread) == 0) {
                thread_to_stack.emplace(std::piecewise_construct, std::forward_as_tuple(thread),
    ...
    public:
         ...
         */
        StackFrameContext(Process& process_, Stack& stack_, const char* function_name_, const char* file_name, size_t line,
                          TypeEraser info)
            : process{process_}
            , stack{stack_}
            , function_name{function_name_}
    ...
    inline CallbackType& Stack::get_callback() const {
        return *process.callback;
    }

    inline CpuTime Stack::get_log_period() const {
        return process.log_period;
    }

    inline WallTime Stack::get_process_start() const {
    ...
    // I believe thread_local StackContainer call create_stack and delete_stack, so I think they will always be called from the
    // target thread. But the main thread might get deleted by the destructor of its containing map, in the main thread.
