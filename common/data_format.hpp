...
struct imu_cam_type : public switchboard::event
{
    ...
    ullong                 dataset_time;
    imu_cam_type(time_type time_, Eigen::Vector3f angular_v_, Eigen::Vector3f linear_a_, std::optional<cv::Mat> img0_,
                 std::optional<cv::Mat> img1_, ullong dataset_time_)
        : time { time_ }
...
public:
    rgb_depth_type(std::optional<cv::Mat> _rgb, std::optional<cv::Mat> _depth, ullong _timestamp)
        : rgb { _rgb }
        , depth { _depth }
...
struct imu_integrator_input : public switchboard::event
{
    ...
    imu_integrator_input(double last_cam_integration_time_, double t_offset_, imu_params params_, Eigen::Vector3d biasAcc_,
                         Eigen::Vector3d biasGyro_, Eigen::Matrix<double, 3, 1> position_,
...
struct imu_raw_type : public switchboard::event
{
    ...
    imu_raw_type(Eigen::Matrix<double, 3, 1> w_hat_, Eigen::Matrix<double, 3, 1> a_hat_, Eigen::Matrix<double, 3, 1> w_hat2_,
                 ...
                 Eigen::Quaterniond quat_, time_type imu_time_)
        : w_hat { w_hat_ }
        , a_hat { a_hat_ }
        , w_hat2 { w_hat2_ }
        , a_hat2 { a_hat2_ }
...
struct rendered_frame : public switchboard::event
{
    ...
    rendered_frame() { }
    rendered_frame(GLuint texture_handles_[2], GLuint swap_indices_[2], fast_pose_type render_pose_, time_type sample_time_,
...
struct texture_pose : public switchboard::event
{
    ...
    texture_pose() { }
    texture_pose(int seq_, int offload_time_, unsigned char* image_, time_type pose_time_, Eigen::Vector3f position_,
