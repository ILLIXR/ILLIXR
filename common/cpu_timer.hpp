...
class timer {
public:
    timer(const now_fn& now, durationt& _duration) : _p_now{now}, _p_duration{_duration} {
        _p_start = _p_now();
    }
...
private:
    ...
    public:
        print_in_destructor(const std::string& account_name, const duration& _duration)
            : _p_account_name{account_name}, _p_duration{_duration} { }
...
public:
    print_timer(const std::string& name, const now_fn& now)
        : _p_print_in_destructor{name, _p_duration}, _p_timer{now, _p_duration} { }
};
