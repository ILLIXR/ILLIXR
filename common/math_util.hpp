...
namespace math_util
{
    ...
    void projection_fov(Eigen::Matrix4f* result, const float fovLeft, const float fovRight, const float fovUp,
                        const float fovDown, const float nearZ, const float farZ);

    // Calculates a projection matrix with given properties. Implementation
    // borrowed from J.M.P. Van Waveren's ksAlgebra library, as found in
    // https://github.com/KhronosGroup/Vulkan-Samples-Deprecated/blob/master/samples/apps/atw/atw_opengl.c
    void projection(Eigen::Matrix4f* result, const float tanAngleLeft, const float tanAngleRight, const float tanAngleUp,
