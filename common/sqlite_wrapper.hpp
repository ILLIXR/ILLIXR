...
namespace sqlite {
    ...
    public:
        ...
        value(value_variant&& data, size_t id = no_id)
            : _m_type{std::holds_alternative<int64_t>(data)                 ? std::cref(type_INTEGER)
        ...
        bool slot_filled() const {
            return !std::holds_alternative<bool>(_m_data) || std::get<bool>(_m_data);
        }

        bool has_data() const {
            return !std::holds_alternative<bool>(_m_data);
        }
    ...
    enum class sentinel { COMMA, LEFT_PARENS, RIGHT_PARENS };
    ...
    public:
        ...
        const std::string_view get_sql() const {
            return sql;
        }
    };
    ...
    private:
        ...
        void validate_literal(const std::string_view literal) {
            ...
            for (const char letter : literal) {
                [[maybe_unused]] bool valid_id = literal.size() > 0 &&
                    (false || (!first && '0' <= letter && letter <= '9') || ('a' <= letter && letter <= 'z') ||
                     ('A' <= letter && letter <= 'Z') || letter == '_');
    ...
    public:
        ...
        statement_builder& operator<<(value&& variable) {
            ...
            assert(!_m_val_map.at(variable.get_id()).has_value() ||
                   _m_val_map.at(variable.get_id())->get_type() == variable.get_type() &&
                       "Type mismatch with prior use of variable");
            assert(!_m_val_map.at(variable.get_id()).has_value() ||
                   _m_val_map.at(variable.get_id())->has_data() && "Cannot repeat the ID of a variable with a value");
