...
public:
     ...
     */
    bool is_set() const { return _m_value; }
     ...
     */
    template <class Clock, class Rep, class Period> bool wait_timeout(const std::chrono::duration<Rep, Period>& duration) const
    {
...
{
public:
    void wait_for_ready() const { _m_ready.wait(); }

    void signal_ready() { _m_ready.set(); }

    bool check_should_stop() const { return _m_should_stop.is_set(); }

    void signal_should_stop() { _m_should_stop.set(); }

    void wait_for_shutdown_complete() const { _m_shutdown_complete.wait(); }

    bool check_shutdown_complete() const { return _m_shutdown_complete.is_set(); }

    void signal_shutdown_complete() { _m_shutdown_complete.set(); }
