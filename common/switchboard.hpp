...
public:
     ...
     */
    template <typename specific_event> using ptr = std::shared_ptr<specific_event>;
     ...
     * \endcode
     */
    template <typename underlying_type> class event_wrapper : public event
    {
    private:
...
private:
    ...
    public:
        topic_subscription(const std::string& topic_name, plugin_id_t plugin_id,
            ...
            , _m_thread { [this] { this->thread_body(); }, [this] { this->thread_on_start(); },
...
public:
     ...
     */
    template <typename specific_event> class reader
    {
    private:
     ...
     */
    template <typename specific_event> class writer
    {
    private:
        // Reference to the underlying topic
        topic& _m_topic;
    ...
    public:
         ...
         */
        template <class... Args> ptr<specific_event> allocate(Args&&... args)
        {
...
private:
    ...
    template <typename specific_event> topic& try_register_topic(const std::string& topic_name)
    {
        {
...
public:
     ...
     * @throws If topic already exists, and its type does not match the @p event.
     */
    template <typename specific_event> writer<specific_event> get_writer(const std::string& topic_name)
    {
     ...
     * @throws If topic already exists, and its type does not match the @p event.
     */
    template <typename specific_event> reader<specific_event> get_reader(const std::string& topic_name)
    {
