...
public:
    ...
    bool                  operator!=(const record_header& other) const { return !(*this == other); }
...
public:
    ...
    template <typename T> T get_value(unsigned column) const
    {
#ifndef NDEBUG
        assert(rh);
...
public:
    ...
    operator bool() const { return bool(logger); }
};
}
