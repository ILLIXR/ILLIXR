...
public:
    record(const record_header& rh_, std::vector<std::any> values_)
	    ...
	    spdlog::get("illixr_file_log")->error("[record_logger] {} elements passed, but rh for {} only specifies {}.",values.size(),
			    rh->get().get_name(), rh->get().get_columns());
            abort();
        }
        for (std::size_t column = 0; column < values.size(); ++column) {
            if (values[column].type() != rh->get().get_column_type(column)) {
		spdlog::get("illixr_file_log")->error("[record_logger] Caller got wrong type for column {} of {}.", column, rh->get().get_name());
		spdlog::get("illixr_file_log")->error("[record_logger] Caller passed: {}; record_header specifies: {}", values[column].type().name(),
    ...
    ~record() {
	    ...
	    spdlog::get("illixr_file_log")->error("[record_logger] Record was deleted without being logged.");
            abort();
        }
#endif
    }
...
public:
    ...
    void log(record r) {
		...
		spdlog::get("illixr_file_log")->error("[record_logger] Tried to push a record of type {} to a record logger for type {}",
