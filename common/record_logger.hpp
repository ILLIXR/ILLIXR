...
public:
    record_header(std::string name_, std::vector<std::pair<std::string, const std::type_info&>> columns_)
        : id{std::hash<std::string>{}(name_)}, name{name_}, columns{columns_} { }
...
class record {
public:
    record(const record_header& rh_, std::vector<std::any> values_) : rh{rh_}, values{values_} {
#ifndef NDEBUG
        assert(rh);
...
public:
    record_coalescer(std::shared_ptr<record_logger> logger_)
        : logger{logger_}, last_log{std::chrono::high_resolution_clock::now()} { }
