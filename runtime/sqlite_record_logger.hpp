...
public:
    ...
    void put_queue(const std::vector<record>& buffer_in) { queue.enqueue_bulk(buffer_in.begin(), buffer_in.size()); }

    void put_queue(const record& record_in) { queue.enqueue(record_in); }
...
protected:
    ...
    virtual void log(const record& r) override { get_sqlite_thread(r).put_queue(r); }
