...
public:
    ...
    void process(cpu_timer::Frames&& finished) {
        ...
        for (const cpu_timer::Frame& frame : finished) {
            const FrameInfo& info = frame.get_info() == cpu_timer::type_eraser_default
                ? default_frame_info
        ...
        std::cout << "Stack frame logger on " << tid << ' ' << "processed " << finished.size() << " frames, "
                  << "conversion: " << size_t(finished.size() * 1e9 / cpu_timer::detail::get_ns(mid - start))
                  << "trans per sec, "
