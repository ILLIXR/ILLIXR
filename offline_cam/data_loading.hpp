...
static std::map<ullong, sensor_types> load_data() {
	...
	spdlog::get("illixr_log")->error("[OFFLINECAM] Please define ILLIXR_DATA");
        ILLIXR::abort();
    }
    ...
    if (!cam0_file.good()) {
	spdlog::get("illixr_log")->error("[OFFLINECAM] ${ILLIXR_DATA} {0} ({1}{0}) is not a good path", cam0_subpath, illixr_data);
        ILLIXR::abort();
    }
    ...
    if (!cam1_file.good()) {
	spdlog::get("illixr_log")->error("[OFFLINECAM] ${ILLIXR_DATA} {0} ({1}{0}) is not a good path", cam1_subpath, illixr_data);
        ILLIXR::abort();
    }
