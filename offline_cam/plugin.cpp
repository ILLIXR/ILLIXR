...
public:
    ...
    #error
            std::this_thread::sleep_for(std::chrono::nanoseconds{nearest_row->first} -
                                        std::chrono::nanoseconds{dataset_first_time} - _m_rtc->time_since_start() -
            ...
            _m_log << _m_rtc->now().time_since_epoch().count() << "," << time_since_start.count() << ","
                   << (nearest_row->first - dataset_first_time) << "\n";
            // CPU_TIMER_TIME_EVENT_INFO(false, false, "entry", cpu_timer::make_type_eraser<FrameInfo>(std::to_string(id),
            // "cam", 0, expected_real_time_given_dataset_time));
            ...
            // 	<< "Diff is " << std::chrono::nanoseconds(time_since_start - std::chrono::nanoseconds{last_ts -
            // dataset_first_time}).count() << "\n"
