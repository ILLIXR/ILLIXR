...
public:
    ...
    ProperQuaternion(const Eigen::Quaternion<Scalar_, Options_>& other)
        : Eigen::Quaternion<Scalar_, Options_>(other) { }

    ProperQuaternion(const Scalar_& w, const Scalar_& x, const Scalar_& y, const Scalar_& z)
    ...
    ProperQuaternion(const Scalar_* data)
        : Eigen::Quaternion<Scalar_, Options_>(data) { }

    ProperQuaternion(const Eigen::Vector<double, 4>& vec)
        : Eigen::Quaternion<Scalar_, Options_>(vec[0], vec[1], vec[2], vec[3]) { }
...
inline Eigen::Vector3d delta_v(const Eigen::Vector3d& iv, const Eigen::Vector3d& k_n) {
