...
//  2023/01/04: fixed a packing issue which in some occurrences would prevent large amount of glyphs from being packed
//  correctly. 2021/08/23: fixed crash when FT_Render_Glyph() fails to render a glyph and returns NULL. 2021/03/05: added
//  ImGuiFreeTypeBuilderFlags_Bitmap to load bitmap glyphs. 2021/03/02: set 'atlas->TexPixelsUseColors = true' to help some
//  backends with deciding of a preferred texture format. 2021/01/28: added support for color-layered glyphs via
//  ImGuiFreeTypeBuilderFlags_LoadColor (require Freetype 2.10+). 2021/01/26: simplified integration by using '#define
//  IMGUI_ENABLE_FREETYPE'. renamed ImGuiFreeType::XXX flags to ImGuiFreeTypeBuilderFlags_XXX for consistency with other API.
//  removed ImGuiFreeType::BuildFontAtlas(). 2020/06/04: fix for rare case where FT_Get_Char_Index() succeed but FT_Load_Glyph()
//  fails. 2019/02/09: added RasterizerFlags::Monochrome flag to disable font anti-aliasing (combine with ::MonoHinting for best
//  results!) 2019/01/15: added support for imgui allocators + added FreeType only override function SetAllocatorFunctions().
//  2019/01/10: re-factored to match big update in STB builder. fixed texture height waste. fixed redundant glyphs when merging.
//  support for glyph padding. 2018/06/08: added support for ImFontConfig::GlyphMinAdvanceX, GlyphMaxAdvanceX. 2018/02/04: moved
//  to main imgui repository (away from http://www.github.com/ocornut/imgui_club) 2018/01/22: fix for addition of
//  ImFontAtlas::TexUvscale member. 2017/10/22: minor inconsequential change to match change in master (removed an unnecessary
//  statement). 2017/09/26: fixes for imgui internal changes. 2017/08/26: cleanup, optimizations, support for
//  ImFontConfig::RasterizerFlags, ImFontConfig::RasterizerMultiply. 2017/08/16: imported from
//  https://github.com/Vuhdo/imgui_freetype into http://www.github.com/ocornut/imgui_club, updated for latest changes in
...
struct FreeTypeFont {
    bool InitFont(FT_Library ft_library, const ImFontConfig& cfg,
                  unsigned int extra_user_flags); // Initialize from an external data buffer. Doesn't copy data, and you must
...
bool FreeTypeFont::InitFont(FT_Library ft_library, const ImFontConfig& cfg, unsigned int extra_font_builder_flags) {
    FT_Error error =
        FT_New_Memory_Face(ft_library, (uint8_t*) cfg.FontData, (uint32_t) cfg.FontDataSize, (uint32_t) cfg.FontNo, &Face);
    if (error != 0)
        return false;
...
void FreeTypeFont::SetPixelHeight(int pixel_height) {
    // Vuhdo: I'm not sure how to deal with font sizes properly. As far as I understand, currently ImGui assumes that the
...
const FT_Glyph_Metrics* FreeTypeFont::LoadGlyph(uint32_t codepoint) {
    ...
    IM_ASSERT(slot->format != FT_GLYPH_FORMAT_SVG &&
              "The font contains SVG glyphs, you'll need to enable IMGUI_ENABLE_FREETYPE_LUNASVG in imconfig.h and install "
              "required libraries in order to use this font");
        #endif
    IM_ASSERT(slot->format == FT_GLYPH_FORMAT_OUTLINE || slot->format == FT_GLYPH_FORMAT_BITMAP);
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags) {
    ...
    // We allocate in chunks of 256 KB to not waste too much extra memory ahead. Hopefully users of FreeType won't mind the
    ...
    stbrp_init_target(&pack_context, atlas->TexWidth - atlas->TexGlyphPadding, TEX_HEIGHT_MAX - atlas->TexGlyphPadding,
...
static FT_Error ImGuiLunasvgPortRender(FT_GlyphSlot slot, FT_Pointer* _state) {
    ...
    // If there was an error while loading the svg in ImGuiLunasvgPortPresetSlot(), the renderer hook still get called, so just
    // returns the error.
