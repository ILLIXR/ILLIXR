...
//  [X] Platform: Partial keyboard support. Since 1.87 we are using the io.AddKeyEvent() function. Pass ImGuiKey values to all
//  key functions e.g. ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy GLUT values will also be supported unless
//  IMGUI_DISABLE_OBSOLETE_KEYIO is set]
// Issues:
...
//  2023-04-17: BREAKING: Removed call to ImGui::NewFrame() from ImGui_ImplGLUT_NewFrame(). Needs to be called from the main
//  application loop, like with every other backends. 2022-09-26: Inputs: Renamed ImGuiKey_ModXXX introduced in 1.87 to
//  ImGuiMod_XXX (old names still supported). 2022-01-26: Inputs: replaced short-lived io.AddKeyModsEvent() (added two weeks
//  ago) with io.AddKeyEvent() using ImGuiKey_ModXXX flags. Sorry for the confusion. 2022-01-17: Inputs: calling new
//  io.AddMousePosEvent(), io.AddMouseButtonEvent(), io.AddMouseWheelEvent() API (1.87+). 2022-01-10: Inputs: calling new
