...
//  https://github.com/ocornut/imgui/pull/914 for discussions. [X] Renderer: Large meshes support (64k+ vertices) with 16-bit
//  indices.
...
// .vcxproj files)
// - [Solution 2] IDE/msbuild: in "Properties/C++/Preprocessor Definitions" add 'IMGUI_USER_CONFIG="my_imgui_config.h"' and
// inside 'my_imgui_config.h' add '#define ImTextureID ImU64' and as many other options as you like.
...
//  2024-04-19: Vulkan: Added convenience support for Volk via IMGUI_IMPL_VULKAN_USE_VOLK define (you can also use
//  IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk via ImGui_ImplVulkan_LoadFunctions().) 2024-02-14: *BREAKING CHANGE*: Moved
//  RenderPass parameter from ImGui_ImplVulkan_Init() function to ImGui_ImplVulkan_InitInfo structure. Not required when using
//  dynamic rendering. 2024-02-12: *BREAKING CHANGE*: Dynamic rendering now require filling PipelineRenderingCreateInfo
//  structure. 2024-01-19: Vulkan: Fixed vkAcquireNextImageKHR() validation errors in VulkanSDK 1.3.275 by allocating one extra
//  semaphore than in-flight frames. (#7236) 2024-01-11: Vulkan: Fixed vkMapMemory() calls unnecessarily using full buffer size
//  (#3957). Fixed MinAllocationSize handing (#7189). 2024-01-03: Vulkan: Added MinAllocationSize field in
//  ImGui_ImplVulkan_InitInfo to workaround zealous "best practice" validation layer. (#7189, #4238) 2024-01-03: Vulkan: Stopped
//  creating command pools with VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT as we don't reset them. 2023-11-29: Vulkan:
//  Fixed mismatching allocator passed to vkCreateCommandPool() vs vkDestroyCommandPool(). (#7075) 2023-11-10: *BREAKING
//  CHANGE*: Removed parameter from ImGui_ImplVulkan_CreateFontsTexture(): backend now creates its own command-buffer to upload
//  fonts.
//              *BREAKING CHANGE*: Removed ImGui_ImplVulkan_DestroyFontUploadObjects() which is now unnecessary as we create and
//              destroy those objects in the backend. ImGui_ImplVulkan_CreateFontsTexture() is automatically called by
//              NewFrame() the first time. You can call ImGui_ImplVulkan_CreateFontsTexture() again to recreate the font atlas
//              texture. Added ImGui_ImplVulkan_DestroyFontsTexture() but you probably never need to call this.
//  2023-07-04: Vulkan: Added optional support for VK_KHR_dynamic_rendering. User needs to set init_info->UseDynamicRendering =
//  true and init_info->ColorAttachmentFormat. 2023-01-02: Vulkan: Fixed sampler passed to ImGui_ImplVulkan_AddTexture() not
//  being honored + removed a bunch of duplicate code. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.
...
//  validation error when buffer sizes are not multiple of VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan:
//  Change blending equation to preserve alpha in output buffer. 2021-01-27: Vulkan: Added support for custom function load and
...
//  frame has no vertices. 2020-04-26: Vulkan: Fixed edge case where render callbacks wouldn't be called if the ImDrawData
//  didn't have vertices. 2019-08-01: Vulkan: Added support for specifying multisample count. Set
//  ImGui_ImplVulkan_InitInfo::MSAASamples to one of the VkSampleCountFlagBits values to use, default is non-multisampled as
//  before. 2019-05-29: Vulkan: Added support for large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset
//  flag. 2019-04-30: Vulkan: Added support for special ImDrawCallback_ResetRenderState callback to reset render state.
//  2019-04-04: *BREAKING CHANGE*: Vulkan: Added ImageCount/MinImageCount fields in ImGui_ImplVulkan_InitInfo, required for
//  initialization (was previously a hard #define IMGUI_VK_QUEUED_FRAMES 2). Added ImGui_ImplVulkan_SetMinImageCount().
...
//  2018-03-03: Vulkan: Various refactor, created a couple of ImGui_ImplVulkanH_XXX helper that the example can use and that
//  viewport support will use. 2018-03-01: Vulkan: Renamed ImGui_ImplVulkan_Init_Info to ImGui_ImplVulkan_InitInfo and fields to
//  match more closely Vulkan terminology. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback,
...
//  2017-05-15: Vulkan: Fix scissor offset being negative. Fix new Vulkan validation warnings. Set required depth member for
//  buffer image copy. 2016-11-13: Vulkan: Fix validation layer warnings and errors and redeclare gl_PerVertex. 2016-10-18:
//  Vulkan: Add location decorators & change to use structs as in/out in glsl, update embedded spv (produced with
//  glslangValidator -x). Null the released resources. 2016-08-27: Vulkan: Fix Vulkan example for use when a depth buffer is
//  active.
...
static void CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory, VkDeviceSize& buffer_size, size_t new_size,
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer,
    ...
    // (bottom right). DisplayPos is (0,0) for single viewport apps.
    {
        ...
        vkCmdPushConstants(command_buffer, bd->PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof(float) * 0, sizeof(float) * 2,
                           scale);
        vkCmdPushConstants(command_buffer, bd->PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof(float) * 2, sizeof(float) * 2,
                           translate);
    }
}
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline) {
    ...
    if (wrb->FrameRenderBuffers == nullptr) {
        ...
        wrb->FrameRenderBuffers =
            (ImGui_ImplVulkan_FrameRenderBuffers*) IM_ALLOC(sizeof(ImGui_ImplVulkan_FrameRenderBuffers) * wrb->Count);
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != nullptr) {
                ...
                if (sizeof(ImTextureID) < sizeof(ImU64)) {
                    // We don't support texture switches if ImTextureID hasn't been redefined to be 64-bit. Do a flaky check
                    // that other textures haven't been used.
                    IM_ASSERT(pcmd->TextureId == (ImTextureID) bd->FontDescriptorSet);
                    desc_set[0] = bd->FontDescriptorSet;
                }
                vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 0, 1, desc_set, 0,
                                        nullptr);
    ...
    // rendering. In theory we should aim to backup/restore those values but I am not sure this is possible. We perform a call
    // to vkCmdSetScissor() to set back a full viewport which is likely to fix things for 99% users but technically this is not
...
bool ImGui_ImplVulkan_CreateFontsTexture() {
    ...
    // Copy to Image:
    {
        ...
        vkCmdPipelineBarrier(bd->FontCommandBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr,
        ...
        vkCmdCopyBufferToImage(bd->FontCommandBuffer, upload_buffer, bd->FontImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                               &region);
        ...
        vkCmdPipelineBarrier(bd->FontCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0,
...
bool ImGui_ImplVulkan_CreateDeviceObjects() {
    ...
    ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache, v->RenderPass, v->MSAASamples, &bd->Pipeline,
                                    v->Subpass);
...
// You may read this code to learn about Vulkan, but it is recommended you use you own custom tailored code to do equivalent
// work. (The ImGui_ImplVulkanH_XXX functions do not interact with any of the state used by the regular ImGui_ImplVulkan_XXX
// functions)
//-------------------------------------------------------------------------
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(VkPhysicalDevice physical_device, VkSurfaceKHR surface,
    ...
    // view format Additionally several new color spaces were introduced with Vulkan Spec v1.0.40, hence we must make sure that
    // a format with the mostly available color space, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR, is found and used.
...
void ImGui_ImplVulkanH_DestroyWindow(VkInstance instance, VkDevice device, ImGui_ImplVulkanH_Window* wd,
                                     const VkAllocationCallbacks* allocator) {
    vkDeviceWaitIdle(device); // FIXME: We could wait on the Queue if we had the queue in wd-> (otherwise VulkanH functions
                              // can't use globals)
