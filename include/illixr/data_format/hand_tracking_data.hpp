...
struct ht_frame : cam_base_type {
    ...
    ht_frame(time_point time, std::map<image::image_type, cv::Mat> images, std::map<units::eyes, ht_detection> _detections,
             ...
             coordinates::reference_space ref_sp = coordinates::VIEWER, units::measurement_unit _unit = units::measurement_unit::UNSET)
...
struct raw_ht_data {
    ...
    raw_ht_data(const time_point time, const std::map<ht::hand, hand_points>& points, const std::map<ht::hand, velocity>& velocities,
                const pose_data& pose, coordinates::reference_space ref_sp = coordinates::VIEWER, units::measurement_unit _unit = units::measurement_unit::UNSET)
    ...
    : raw_ht_data(frame_.time, frame_.hand_positions, frame_.hand_velocities, frame_.wcs_offset, frame_.reference, frame_.unit) {}
...
inline std::ostream &operator<<(std::ostream &os, const raw_ht_data& data) {
    ...
    if (data.wcs_origin.valid) {
        os << std::endl << "    position: " << data.wcs_origin.x << ", " << data.wcs_origin.y << ", " << data.wcs_origin.z << std::endl;
        os << "    orientation: " << data.wcs_origin.w << ", " << data.wcs_origin.wx << ", " << data.wcs_origin.wy << ", " << data.wcs_origin.z;
    } else {
        os << "not valid";
    }
    ...
    if (data.hp_valid[LEFT_HAND]) {
        for (auto i = 0; i < NUM_LANDMARKS; i++){
            os << std::endl << "    " << i << ": " << data.h_points[LEFT_HAND][i].x << ", " << data.h_points[LEFT_HAND][i].y
    ...
    if (data.hp_valid[RIGHT_HAND]) {
        for (auto i = 0; i < NUM_LANDMARKS; i++){
            os << std::endl << "    " << i << ": " << data.h_points[RIGHT_HAND][i].x << ", " << data.h_points[RIGHT_HAND][i].y
    ...
    if (data.hv_valid[LEFT_HAND]) {
        for (auto i = 0; i < NUM_LANDMARKS; i++){
            os << std::endl << "    " << i << ": " << data.h_velocities[LEFT_HAND][i].x << ", " << data.h_velocities[LEFT_HAND][i].y
               << ", " << data.h_velocities[LEFT_HAND][i].z << "  " << ((data.h_velocities[LEFT_HAND][i].valid)? "valid" : "not valid");
        }
    } else {
        os << "not valid";
    }
    ...
    if (data.hv_valid[RIGHT_HAND]) {
        for (auto i = 0; i < NUM_LANDMARKS; i++){
            os << std::endl << "    " << i << ": " << data.h_velocities[RIGHT_HAND][i].x << ", " << data.h_velocities[RIGHT_HAND][i].y
               << ", " << data.h_velocities[RIGHT_HAND][i].z << "  " << ((data.h_velocities[RIGHT_HAND][i].valid)? "valid" : "not valid");
        }
    } else {
        os << "not valid";
    }
