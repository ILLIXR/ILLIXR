...
namespace ht {
    ...
    struct ht_frame : cam_base_type {
        ...
        head_pose_data                      wcs_offset;      //!< The offset between the current pose and the origin
        ...
        ht_frame(time_point time, std::map<image::image_type, cv::Mat> images, std::map<units::eyes, ht_detection> _detections,
                 std::map<ht::hand, hand_points> points, std::map<ht::hand, velocity> velocities = {},
                 head_pose_data _pose = {}, coordinates::reference_space ref_sp = coordinates::VIEWER,
    ...
    struct raw_ht_data {
        ...
        raw_ht_data(const time_point time, const std::map<ht::hand, hand_points>& points,
                    const std::map<ht::hand, velocity>& velocities, const head_pose_data& pose,
     ...
     */
    [[maybe_unused]] inline void transform_point(point_with_units& pnt, const head_pose_data& pose) {
     ...
     */
    [[maybe_unused]] inline static void transform_points(points_with_units& points, const head_pose_data& pose,
