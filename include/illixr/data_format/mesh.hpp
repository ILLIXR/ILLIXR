...
struct mesh_type : public switchboard::event {
    ...
    [[maybe_unused]]mesh_type(const unsigned type_, std::unique_ptr<draco_illixr::PlyReader> input_reader, unsigned id_)
            : type{type_}
            , id{id_}
            , reader{std::move(input_reader)} { }

    [[maybe_unused]]mesh_type(const unsigned type_,std::unique_ptr<draco_illixr::PlyReader> input_reader, unsigned id_, bool is_active)
            : type{type_}
            , active{is_active}
            , id{id_}
            , reader{std::move(input_reader)}{ }
    ...
    [[maybe_unused]]mesh_type(const unsigned type_, std::unique_ptr<draco_illixr::PlyReader> input_reader, bool is_compressed,
