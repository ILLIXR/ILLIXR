...
struct ccd_data {
    ...
    ccd_data(const float cx, const float cy, const double vf, const double hf)
        : center_x{cx}
        , center_y{cy}
        , vertical_fov{(vf > T_M_PI) ? vf * M_PI / 180. : vf}  // assume any value below 2PI is in radians, anything else is degrees
        , horizontal_fov{(hf > T_M_PI) ? hf * M_PI / 180. : hf} {}
};

typedef std::map<units::eyes, ccd_data> ccd_map; //!< mapping of ccd information to the eye it is associated with, for monocular cameras use `units::LEFT_EYE`
