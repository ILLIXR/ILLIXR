 ...
 * @throws std::runtime_error on any JNI lookup failure.
    ...
    std::string invoke_java_dialog(struct android_app* app, const std::string& profiles_dir) {
        ...
        auto result_jstring = reinterpret_cast<jstring>(
            env->CallStaticObjectMethod(dialog_class, show_method,
                                        activity_object, j_profiles_dir));
    ...
    int extract_profiles(AAssetManager* asset_manager, const std::string& dest_dir) {
        ...
        if (asset_dir == nullptr) {
            spdlog::get("illixr")->error(
                "[profile_picker_dialog] No 'profiles' directory found in APK assets");
            return -1;
        }
        ...
        while ((filename = AAssetDir_getNextFileName(asset_dir)) != nullptr) {
            ...
            if (asset == nullptr) {
                spdlog::get("illixr")->warn(
                    "[profile_picker_dialog] Could not open asset: {}", asset_path);
                continue;
            }
            ...
            if (fp != nullptr) {
                ...
                spdlog::get("illixr")->warn(
                    "[profile_picker_dialog] Could not write: {}", dest_path);
            }
...
std::string show_profile_picker_dialog(struct android_app* app) {
    ...
    const std::string profiles_dir =
        std::string(native_activity->internalDataPath) + "/profiles";
    ...
    if (extracted < 0) {
        spdlog::get("illixr")->error(
            "[profile_picker_dialog] Failed to extract profiles from APK assets");
        return "";
    }
    if (extracted == 0) {
        spdlog::get("illixr")->error(
            "[profile_picker_dialog] No profile files found in APK assets/profiles/");
        return "";
    }

    spdlog::get("illixr")->info(
        "[profile_picker_dialog] Extracted {} profile(s) to {}", extracted, profiles_dir);
    ...
    if (chosen.empty()) {
        spdlog::get("illixr")->warn(
            "[profile_picker_dialog] User cancelled profile selection.");
    } else {
        spdlog::get("illixr")->info(
            "[profile_picker_dialog] Profile selected: {}", chosen);
    }
