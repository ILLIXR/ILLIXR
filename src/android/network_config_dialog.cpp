    ...
    std::string invoke_java_dialog(struct android_app* app, bool use_tcp, bool use_udp) {
        ...
        if (dialog_class == nullptr) {
            ...
            throw std::runtime_error(
                "[network_config_dialog] Could not find class "
        ...
        jmethodID show_method = env->GetStaticMethodID(
            dialog_class, "showDialog", "(Landroid/app/Activity;ZZ)Ljava/lang/String;");
        if (show_method == nullptr) {
            env->DeleteLocalRef(dialog_class);
            jvm->DetachCurrentThread();
            throw std::runtime_error(
                "[network_config_dialog] Could not find method "
        ...
        auto result_jstring  = reinterpret_cast<jstring>(env->CallStaticObjectMethod(
            dialog_class, show_method, activity_object,
            static_cast<jboolean>(use_tcp), static_cast<jboolean>(use_udp)));
...
network_config show_network_config_dialog(struct android_app* app,
    ...
    if (raw.empty()) {
        spdlog::get("illixr")->warn(
            "[network_config_dialog] User cancelled network configuration.");
        return cfg; // cancelled = true
    }
    ...
    const int expected = 3 * (flags.use_tcp ? 1 : 0)
        + 3 * (flags.use_udp ? 1 : 0)
        + 1;
    std::vector<std::string> parts = split(raw, '|');
    if (static_cast<int>(parts.size()) != expected) {
        spdlog::get("illixr")->error(
            "[network_config_dialog] Unexpected result field count {} (expected {}): '{}'",
    ...
    spdlog::get("illixr")->info(
        "[network_config_dialog] Configuration applied: "
